...
loadjs.ready("head", function () {
    ...
    fAGEedit = currentForm = new ew.Form("fAGEedit", "edit");
    ...
    // Form_CustomValidate
    fAGEedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fAGEedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fAGEedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fAGEedit" id="fAGEedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="AGE" data-field="x_CAT" data-hidden="1" name="o_CAT" id="o_CAT" value="<?= HtmlEncode($Page->CAT->OldValue ?? $Page->CAT->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="AGE" data-field="x_GROUP_ID" data-hidden="1" name="o_GROUP_ID" id="o_GROUP_ID" value="<?= HtmlEncode($Page->GROUP_ID->OldValue ?? $Page->GROUP_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
