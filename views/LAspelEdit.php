...
loadjs.ready("head", function () {
    ...
    fl_aspeledit = currentForm = new ew.Form("fl_aspeledit", "edit");
    ...
    fl_aspeledit.addFields([
        ["id_aspel", [fields.id_aspel.visible && fields.id_aspel.required ? ew.Validators.required(fields.id_aspel.caption) : null], fields.id_aspel.isInvalid],
    ...
    // Validate form
    fl_aspeledit.validate = function () {
    ...
    // Form_CustomValidate
    fl_aspeledit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fl_aspeledit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fl_aspeledit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fl_aspeledit" id="fl_aspeledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_aspel->Visible) { // id_aspel ?>
    <div id="r_id_aspel" class="form-group row">
        <label id="elh_l_aspel_id_aspel" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_aspel->caption() ?><?= $Page->id_aspel->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_aspel->getDisplayValue($Page->id_aspel->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="l_aspel" data-field="x_id_aspel" data-hidden="1" name="x_id_aspel" id="x_id_aspel" value="<?= HtmlEncode($Page->id_aspel->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
