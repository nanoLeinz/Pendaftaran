...
// Page object
$RegisterPerpoliTahunanSummary = &$Page;
?>
...
loadjs.ready("head", function () {
    ...
    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "register_perpoli_tahunan")) ?>,
...
<input type="hidden" name="t" value="register_perpoli_tahunan">
...
<div id="xsr_<?= $Page->SearchRowCount ?>" class="ew-row d-sm-flex">
    ...
    <div id="xsc_YEAR" class="ew-cell form-group">
        ...
        <span id="el_register_perpoli_tahunan_YEAR" class="ew-search-field">
    <select
        id="x_YEAR"
        name="x_YEAR"
        class="form-control ew-select<?= $Page->YEAR->isInvalidClass() ?>"
        data-select2-id="register_perpoli_tahunan_x_YEAR"
        data-table="register_perpoli_tahunan"
        data-field="x_YEAR"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='register_perpoli_tahunan_x_YEAR']"),
        options = { name: "x_YEAR", selectId: "register_perpoli_tahunan_x_YEAR", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.register_perpoli_tahunan.fields.YEAR.selectOptions);
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_register_perpoli_tahunan" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<table class="<?= $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
    <tr class="ew-table-header">
<?php if ($Page->NO_REGISTRATION->Visible) { ?>
    <th data-name="NO_REGISTRATION" class="<?= $Page->NO_REGISTRATION->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_NO_REGISTRATION"><?= $Page->renderSort($Page->NO_REGISTRATION) ?></div></th>
<?php } ?>
<?php if ($Page->VISIT_DATE->Visible) { ?>
    <th data-name="VISIT_DATE" class="<?= $Page->VISIT_DATE->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_VISIT_DATE"><?= $Page->renderSort($Page->VISIT_DATE) ?></div></th>
<?php } ?>
<?php if ($Page->PAYOR_ID->Visible) { ?>
    <th data-name="PAYOR_ID" class="<?= $Page->PAYOR_ID->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_PAYOR_ID"><?= $Page->renderSort($Page->PAYOR_ID) ?></div></th>
<?php } ?>
<?php if ($Page->CLINIC_ID->Visible) { ?>
    <th data-name="CLINIC_ID" class="<?= $Page->CLINIC_ID->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_CLINIC_ID"><?= $Page->renderSort($Page->CLINIC_ID) ?></div></th>
<?php } ?>
<?php if ($Page->MONTH->Visible) { ?>
    <th data-name="MONTH" class="<?= $Page->MONTH->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_MONTH"><?= $Page->renderSort($Page->MONTH) ?></div></th>
<?php } ?>
<?php if ($Page->YEAR->Visible) { ?>
    <th data-name="YEAR" class="<?= $Page->YEAR->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_perpoli_tahunan_YEAR"><?= $Page->renderSort($Page->YEAR) ?></div></th>
<?php } ?>
    </tr>
</thead>
<tbody>
