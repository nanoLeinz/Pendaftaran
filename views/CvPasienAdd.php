...
loadjs.ready("head", function () {
    ...
    fcv_pasienadd.lists.STATUS_PASIEN_ID = <?= $Page->STATUS_PASIEN_ID->toClientList($Page) ?>;
...
loadjs.ready(["fcv_pasienadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fcv_pasienadd", "x_REGISTRATION_DATE", {"ignoreReadonly":true,"useCurrent":false,"format":11});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    <span id="el_cv_pasien_newapp">
    <input type="hidden" data-table="cv_pasien" data-field="x_newapp" data-hidden="1" name="x_newapp" id="x_newapp" value="<?= HtmlEncode($Page->newapp->CurrentValue) ?>">
    </span>
</div><!-- /page* -->
<?php
