...
loadjs.ready("head", function () {
    ...
    fm_jadwaledit = currentForm = new ew.Form("fm_jadwaledit", "edit");
    ...
    // Validate form
    fm_jadwaledit.validate = function () {
    ...
    // Form_CustomValidate
    fm_jadwaledit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fm_jadwaledit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fm_jadwaledit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fm_jadwaledit" id="fm_jadwaledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_jadwal->Visible) { // id_jadwal ?>
    <div id="r_id_jadwal" class="form-group row">
        <label id="elh_m_jadwal_id_jadwal" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_jadwal->caption() ?><?= $Page->id_jadwal->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_jadwal->getDisplayValue($Page->id_jadwal->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="m_jadwal" data-field="x_id_jadwal" data-hidden="1" name="x_id_jadwal" id="x_id_jadwal" value="<?= HtmlEncode($Page->id_jadwal->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
