...
// Page object
$LFlagprocedureEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fl_flagprocedureedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fl_flagprocedureedit = currentForm = new ew.Form("fl_flagprocedureedit", "edit");
    ...
    fl_flagprocedureedit.addFields([
        ["id_procedure", [fields.id_procedure.visible && fields.id_procedure.required ? ew.Validators.required(fields.id_procedure.caption) : null], fields.id_procedure.isInvalid],
    ...
    // Validate form
    fl_flagprocedureedit.validate = function () {
    ...
    // Form_CustomValidate
    fl_flagprocedureedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fl_flagprocedureedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fl_flagprocedureedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fl_flagprocedureedit" id="fl_flagprocedureedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_procedure->Visible) { // id_procedure ?>
    <div id="r_id_procedure" class="form-group row">
        <label id="elh_l_flagprocedure_id_procedure" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_procedure->caption() ?><?= $Page->id_procedure->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_procedure->getDisplayValue($Page->id_procedure->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="l_flagprocedure" data-field="x_id_procedure" data-hidden="1" name="x_id_procedure" id="x_id_procedure" value="<?= HtmlEncode($Page->id_procedure->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
