...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID" class="form-group row">
        <label id="elh_V_RAWAT_INAP_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->ID->cellAttributes() ?>>
<span id="el_V_RAWAT_INAP_ID">
<input type="<?= $Page->ID->getInputTextType() ?>" data-table="V_RAWAT_INAP" data-field="x_ID" name="x_ID" id="x_ID" placeholder="<?= HtmlEncode($Page->ID->getPlaceHolder()) ?>" value="<?= $Page->ID->EditValue ?>"<?= $Page->ID->editAttributes() ?> aria-describedby="x_ID_help">
