...
<?php if ($Page->ID->Visible) { // ID ?>
    <div id="r_ID" class="form-group row">
        <label id="elh_ANTRIAN_LOGIN_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ID->caption() ?><?= $Page->ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<script>
loadjs.ready(["fANTRIAN_LOGINedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fANTRIAN_LOGINedit", "x_TANGGAL_REGIS", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
