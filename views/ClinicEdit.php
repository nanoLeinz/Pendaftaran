...
loadjs.ready("head", function () {
    ...
    fCLINICedit = currentForm = new ew.Form("fCLINICedit", "edit");
    ...
    // Form_CustomValidate
    fCLINICedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fCLINICedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fCLINICedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fCLINICedit" id="fCLINICedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="CLINIC" data-field="x_ORG_UNIT_CODE" data-hidden="1" name="o_ORG_UNIT_CODE" id="o_ORG_UNIT_CODE" value="<?= HtmlEncode($Page->ORG_UNIT_CODE->OldValue ?? $Page->ORG_UNIT_CODE->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="CLINIC" data-field="x_CLINIC_ID" data-hidden="1" name="o_CLINIC_ID" id="o_CLINIC_ID" value="<?= HtmlEncode($Page->CLINIC_ID->OldValue ?? $Page->CLINIC_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
