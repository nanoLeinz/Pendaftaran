...
loadjs.ready("head", function () {
    ...
    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "register_cara_bayar")) ?>,
    ...
    fsummary.lists.STATUS_PASIEN_ID = <?= $Page->STATUS_PASIEN_ID->toClientList($Page) ?>;
...
<input type="hidden" name="t" value="register_cara_bayar">
...
<input type="hidden" name="z_VISIT_DATE" id="z_VISIT_DATE" value="=">
        ...
        <span id="el_register_cara_bayar_VISIT_DATE" class="ew-search-field">
<input type="<?= $Page->VISIT_DATE->getInputTextType() ?>" data-table="register_cara_bayar" data-field="x_VISIT_DATE" name="x_VISIT_DATE" id="x_VISIT_DATE" placeholder="<?= HtmlEncode($Page->VISIT_DATE->getPlaceHolder()) ?>" value="<?= $Page->VISIT_DATE->EditValue ?>"<?= $Page->VISIT_DATE->editAttributes() ?>>
...
<div id="xsr_<?= $Page->SearchRowCount ?>" class="ew-row d-sm-flex">
    ...
    <div id="xsc_STATUS_PASIEN_ID" class="ew-cell form-group">
        ...
        <span id="el_register_cara_bayar_STATUS_PASIEN_ID" class="ew-search-field">
    <select
        id="x_STATUS_PASIEN_ID"
        name="x_STATUS_PASIEN_ID"
        class="form-control ew-select<?= $Page->STATUS_PASIEN_ID->isInvalidClass() ?>"
        data-select2-id="register_cara_bayar_x_STATUS_PASIEN_ID"
        data-table="register_cara_bayar"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='register_cara_bayar_x_STATUS_PASIEN_ID']"),
        options = { name: "x_STATUS_PASIEN_ID", selectId: "register_cara_bayar_x_STATUS_PASIEN_ID", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.register_cara_bayar.fields.STATUS_PASIEN_ID.selectOptions);
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_register_cara_bayar" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
<table class="<?= $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
    <tr class="ew-table-header">
<?php if ($Page->VISIT_DATE->Visible) { ?>
    <th data-name="VISIT_DATE" class="<?= $Page->VISIT_DATE->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_cara_bayar_VISIT_DATE"><?= $Page->renderSort($Page->VISIT_DATE) ?></div></th>
<?php } ?>
<?php if ($Page->NO_REGISTRATION->Visible) { ?>
    <th data-name="NO_REGISTRATION" class="<?= $Page->NO_REGISTRATION->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_cara_bayar_NO_REGISTRATION"><?= $Page->renderSort($Page->NO_REGISTRATION) ?></div></th>
<?php } ?>
<?php if ($Page->GENDER->Visible) { ?>
    <th data-name="GENDER" class="<?= $Page->GENDER->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_cara_bayar_GENDER"><?= $Page->renderSort($Page->GENDER) ?></div></th>
<?php } ?>
<?php if ($Page->STATUS_PASIEN_ID->Visible) { ?>
    <th data-name="STATUS_PASIEN_ID" class="<?= $Page->STATUS_PASIEN_ID->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_cara_bayar_STATUS_PASIEN_ID"><?= $Page->renderSort($Page->STATUS_PASIEN_ID) ?></div></th>
<?php } ?>
<?php if ($Page->NAME_OF_CLINIC->Visible) { ?>
    <th data-name="NAME_OF_CLINIC" class="<?= $Page->NAME_OF_CLINIC->headerCellClass() ?>" style="white-space: nowrap;"><div class="register_cara_bayar_NAME_OF_CLINIC"><?= $Page->renderSort($Page->NAME_OF_CLINIC) ?></div></th>
<?php } ?>
    </tr>
</thead>
<tbody>
