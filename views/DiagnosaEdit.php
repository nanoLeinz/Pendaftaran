...
loadjs.ready("head", function () {
    ...
    fDIAGNOSAedit = currentForm = new ew.Form("fDIAGNOSAedit", "edit");
    ...
    // Form_CustomValidate
    fDIAGNOSAedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fDIAGNOSAedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fDIAGNOSAedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fDIAGNOSAedit" id="fDIAGNOSAedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="DIAGNOSA" data-field="x_DIAGNOSA_ID" data-hidden="1" name="o_DIAGNOSA_ID" id="o_DIAGNOSA_ID" value="<?= HtmlEncode($Page->DIAGNOSA_ID->OldValue ?? $Page->DIAGNOSA_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
