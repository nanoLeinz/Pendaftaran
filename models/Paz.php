...
class Paz extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // no_registration
        $this->no_registration = new DbField('paz', 'paz', 'x_no_registration', 'no_registration', '[no_registration]', '[no_registration]', 200, 25, -1, false, '[no_registration]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // jml
        $this->jml = new DbField('paz', 'paz', 'x_jml', 'jml', '[jml]', 'CAST([jml] AS NVARCHAR)', 3, 4, -1, false, '[jml]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[paz]";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PazList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PazEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PazAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PazView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PazView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PazAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PazAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("PazEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PazAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PazDelete", $this->getUrlParm());
    }
