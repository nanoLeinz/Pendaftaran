 ...
 * Page class
 */
class VRiwayatRmList extends VRiwayatRm
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'V_RIWAYAT_RM';

    // Page object name
    public $PageObjName = "VRiwayatRmList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_RIWAYAT_RMlist";
    ...
    public function __construct()
    {
        ...
        // Table object (V_RIWAYAT_RM)
        if (!isset($GLOBALS["V_RIWAYAT_RM"]) || get_class($GLOBALS["V_RIWAYAT_RM"]) == PROJECT_NAMESPACE . "V_RIWAYAT_RM") {
            $GLOBALS["V_RIWAYAT_RM"] = &$this;
        }
        ...
        $this->AddUrl = "VRiwayatRmAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VRiwayatRmDelete";
        $this->MultiUpdateUrl = "VRiwayatRmUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'V_RIWAYAT_RM');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fV_RIWAYAT_RMlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("V_RIWAYAT_RM"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fV_RIWAYAT_RMlistsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_PASIEN_DIAGNOSA"
        $item = &$this->ListOptions->add("detail_PASIEN_DIAGNOSA");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'PASIEN_DIAGNOSA') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_PASIEN_DIAGNOSA"
        $opt = $this->ListOptions["detail_PASIEN_DIAGNOSA"];
        if ($Security->allowList(CurrentProjectID() . 'PASIEN_DIAGNOSA')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("PASIEN_DIAGNOSA", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("PasienDiagnosaList?" . Config("TABLE_SHOW_MASTER") . "=V_RIWAYAT_RM&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fV_RIWAYAT_RMlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fV_RIWAYAT_RMlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fV_RIWAYAT_RMlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fV_RIWAYAT_RMlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
