 ...
 * Page class
 */
class VRadiologiSearch extends VRadiologi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'V_RADIOLOGI';

    // Page object name
    public $PageObjName = "VRadiologiSearch";
    ...
    public function __construct()
    {
        ...
        // Table object (V_RADIOLOGI)
        if (!isset($GLOBALS["V_RADIOLOGI"]) || get_class($GLOBALS["V_RADIOLOGI"]) == PROJECT_NAMESPACE . "V_RADIOLOGI") {
            $GLOBALS["V_RADIOLOGI"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'V_RADIOLOGI');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("V_RADIOLOGI"));
                $doc->Text = @$content;
        ...
        if ($this->isPageRequest()) {
            ...
            if ($this->isSearch()) {
                ...
                if ($srchStr != "") {
                    ...
                    $srchStr = "VRadiologiList" . "?" . $srchStr;
    ...
    protected function loadSearchValues()
    {
        ...
        if (is_array($this->COB->AdvancedSearch->SearchValue)) {
            $this->COB->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->COB->AdvancedSearch->SearchValue);
        }
        if (is_array($this->COB->AdvancedSearch->SearchValue2)) {
            $this->COB->AdvancedSearch->SearchValue2 = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->COB->AdvancedSearch->SearchValue2);
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                $this->RUJUKAN_ID->AdvancedSearch->ViewValue = $this->RUJUKAN_ID->lookupCacheOption($curVal);
            } else {
                $this->RUJUKAN_ID->AdvancedSearch->ViewValue = $this->RUJUKAN_ID->Lookup !== null && is_array($this->RUJUKAN_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->RUJUKAN_ID->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->RUJUKAN_ID->EditValue = array_values($this->RUJUKAN_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[RUJUKAN_ID]" . SearchString("=", $this->RUJUKAN_ID->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
            ...
            if ($curVal != "") {
                $this->PATIENT_CATEGORY_ID->AdvancedSearch->ViewValue = $this->PATIENT_CATEGORY_ID->lookupCacheOption($curVal);
            } else {
                $this->PATIENT_CATEGORY_ID->AdvancedSearch->ViewValue = $this->PATIENT_CATEGORY_ID->Lookup !== null && is_array($this->PATIENT_CATEGORY_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->PATIENT_CATEGORY_ID->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->PATIENT_CATEGORY_ID->EditValue = array_values($this->PATIENT_CATEGORY_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[PATIENT_CATEGORY_ID]" . SearchString("=", $this->PATIENT_CATEGORY_ID->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->PATIENT_CATEGORY_ID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->PATIENT_CATEGORY_ID->EditValue = $arwrk;
            }
            ...
            $this->BOOKED_DATE->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->BOOKED_DATE->AdvancedSearch->SearchValue, 0), 8));
            ...
            if ($curVal != "") {
                $this->CLINIC_ID_FROM->AdvancedSearch->ViewValue = $this->CLINIC_ID_FROM->lookupCacheOption($curVal);
            } else {
                $this->CLINIC_ID_FROM->AdvancedSearch->ViewValue = $this->CLINIC_ID_FROM->Lookup !== null && is_array($this->CLINIC_ID_FROM->Lookup->Options) ? $curVal : null;
            }
            if ($this->CLINIC_ID_FROM->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->CLINIC_ID_FROM->EditValue = array_values($this->CLINIC_ID_FROM->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[CLINIC_ID]" . SearchString("=", $this->CLINIC_ID_FROM->AdvancedSearch->SearchValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->CLINIC_ID_FROM->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->CLINIC_ID_FROM->EditValue = $arwrk;
            }
            ...
            $this->IN_DATE->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->IN_DATE->AdvancedSearch->SearchValue, 0), 8));
            ...
            $this->FAMILY_STATUS_ID->EditValue = HtmlEncode($this->FAMILY_STATUS_ID->AdvancedSearch->SearchValue);
            ...
            if (!$this->ISATTENDED->Raw) {
                $this->ISATTENDED->AdvancedSearch->SearchValue = HtmlDecode($this->ISATTENDED->AdvancedSearch->SearchValue);
            }
            $this->ISATTENDED->EditValue = HtmlEncode($this->ISATTENDED->AdvancedSearch->SearchValue);
            ...
            if ($curVal != "") {
                $this->PAYOR_ID->AdvancedSearch->ViewValue = $this->PAYOR_ID->lookupCacheOption($curVal);
            } else {
                $this->PAYOR_ID->AdvancedSearch->ViewValue = $this->PAYOR_ID->Lookup !== null && is_array($this->PAYOR_ID->Lookup->Options) ? $curVal : null;
            }
            if ($this->PAYOR_ID->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->PAYOR_ID->EditValue = array_values($this->PAYOR_ID->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[PAYOR_ID]" . SearchString("=", $this->PAYOR_ID->AdvancedSearch->SearchValue, DATATYPE_STRING, "");
                }
            ...
            $this->tanggal_rujukan->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->tanggal_rujukan->AdvancedSearch->SearchValue, 0), 8));
            ...
            $curVal = trim(strval($this->ISRJ->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->ISRJ->AdvancedSearch->ViewValue = $this->ISRJ->lookupCacheOption($curVal);
            } else {
                $this->ISRJ->AdvancedSearch->ViewValue = $this->ISRJ->Lookup !== null && is_array($this->ISRJ->Lookup->Options) ? $curVal : null;
            }
            if ($this->ISRJ->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->ISRJ->EditValue = array_values($this->ISRJ->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[KELUAR_ID]" . SearchString("=", $this->ISRJ->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->ISRJ->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->ISRJ->EditValue = $arwrk;
            }
            ...
            if ($curVal != "") {
                $this->PPKRUJUKAN->AdvancedSearch->ViewValue = $this->PPKRUJUKAN->lookupCacheOption($curVal);
            } else {
                $this->PPKRUJUKAN->AdvancedSearch->ViewValue = $this->PPKRUJUKAN->Lookup !== null && is_array($this->PPKRUJUKAN->Lookup->Options) ? $curVal : null;
            }
            if ($this->PPKRUJUKAN->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->PPKRUJUKAN->EditValue = array_values($this->PPKRUJUKAN->Lookup->Options);
                if ($this->PPKRUJUKAN->AdvancedSearch->ViewValue == "") {
                    $this->PPKRUJUKAN->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[KDPROVIDER]" . SearchString("=", $this->PPKRUJUKAN->AdvancedSearch->SearchValue, DATATYPE_STRING, "");
                }
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->PPKRUJUKAN->Lookup->renderViewRow($rswrk[0]);
                    $this->PPKRUJUKAN->AdvancedSearch->ViewValue = $this->PPKRUJUKAN->displayValue($arwrk);
                } else {
                    $this->PPKRUJUKAN->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            if (!$this->KDPOLI_EKS->Raw) {
                $this->KDPOLI_EKS->AdvancedSearch->SearchValue = HtmlDecode($this->KDPOLI_EKS->AdvancedSearch->SearchValue);
            }
            $this->KDPOLI_EKS->EditValue = HtmlEncode($this->KDPOLI_EKS->AdvancedSearch->SearchValue);
            ...
            if (!$this->ASALRUJUKAN->Raw) {
                $this->ASALRUJUKAN->AdvancedSearch->SearchValue = HtmlDecode($this->ASALRUJUKAN->AdvancedSearch->SearchValue);
            }
            $this->ASALRUJUKAN->EditValue = HtmlEncode($this->ASALRUJUKAN->AdvancedSearch->SearchValue);
            ...
            if (!$this->RESPONTGLPLG_DESC->Raw) {
                $this->RESPONTGLPLG_DESC->AdvancedSearch->SearchValue = HtmlDecode($this->RESPONTGLPLG_DESC->AdvancedSearch->SearchValue);
            }
            $this->RESPONTGLPLG_DESC->EditValue = HtmlEncode($this->RESPONTGLPLG_DESC->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->BOOKED_DATE->AdvancedSearch->SearchValue)) {
            $this->BOOKED_DATE->addErrorMessage($this->BOOKED_DATE->getErrorMessage(false));
        }
        if (!CheckDate($this->VISIT_DATE->AdvancedSearch->SearchValue)) {
            $this->VISIT_DATE->addErrorMessage($this->VISIT_DATE->getErrorMessage(false));
        }
        if (!CheckInteger($this->FOLLOW_UP->AdvancedSearch->SearchValue)) {
            $this->FOLLOW_UP->addErrorMessage($this->FOLLOW_UP->getErrorMessage(false));
        }
        if (!CheckInteger($this->PLACE_TYPE->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckDate($this->IN_DATE->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckInteger($this->RESPONSIBLE_ID->AdvancedSearch->SearchValue)) {
            $this->RESPONSIBLE_ID->addErrorMessage($this->RESPONSIBLE_ID->getErrorMessage(false));
        }
        if (!CheckInteger($this->FAMILY_STATUS_ID->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckDate($this->tanggal_rujukan->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckInteger($this->CALL_TIMES->AdvancedSearch->SearchValue)) {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("VRadiologiList"), "", $this->TableVar, true);
        $pageId = "search";
