...
/**
 * Table class for KALURAHAN
 */
class Kalurahan extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'KALURAHAN';
        $this->TableName = 'KALURAHAN';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[KALURAHAN]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // KAL_ID
        $this->KAL_ID = new DbField('KALURAHAN', 'KALURAHAN', 'x_KAL_ID', 'KAL_ID', '[KAL_ID]', '[KAL_ID]', 200, 50, -1, false, '[KAL_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // KALURAHAN
        $this->KALURAHAN = new DbField('KALURAHAN', 'KALURAHAN', 'x_KALURAHAN', 'KALURAHAN', '[KALURAHAN]', '[KALURAHAN]', 200, 200, -1, false, '[KALURAHAN]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // KEC_ID
        $this->KEC_ID = new DbField('KALURAHAN', 'KALURAHAN', 'x_KEC_ID', 'KEC_ID', '[KEC_ID]', '[KEC_ID]', 200, 50, -1, false, '[KEC_ID]', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        switch ($CurrentLanguage) {
            case "en":
                $this->KEC_ID->Lookup = new Lookup('KEC_ID', 'KECAMATAN', false, 'KEC_ID', ["KECAMATAN","","",""], [], [], [], [], [], [], '', '');
                break;
            default:
                $this->KEC_ID->Lookup = new Lookup('KEC_ID', 'KECAMATAN', false, 'KEC_ID', ["KECAMATAN","","",""], [], [], [], [], [], [], '', '');
                break;
        ...
        // iswilayah
        $this->iswilayah = new DbField('KALURAHAN', 'KALURAHAN', 'x_iswilayah', 'iswilayah', '[iswilayah]', '[iswilayah]', 129, 10, -1, false, '[iswilayah]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->iswilayah->Sortable = true; // Allow sort
        $this->iswilayah->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->iswilayah->Param, "CustomMsg");
        $this->Fields['iswilayah'] = &$this->iswilayah;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[KALURAHAN]";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->iswilayah->DbValue = $row['iswilayah'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("KalurahanList");
    }
    ...
    {
        global $Language;
        if ($pageName == "KalurahanView") {
            return $Language->phrase("View");
        } elseif ($pageName == "KalurahanEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "KalurahanAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "KalurahanView";
            case Config("API_ADD_ACTION"):
                return "KalurahanAdd";
            case Config("API_EDIT_ACTION"):
                return "KalurahanEdit";
            case Config("API_DELETE_ACTION"):
                return "KalurahanDelete";
            case Config("API_LIST_ACTION"):
                return "KalurahanList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("KalurahanView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("KalurahanView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "KalurahanAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "KalurahanAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("KalurahanEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("KalurahanAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("KalurahanDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->KAL_ID->CurrentValue = $key;
            } else {
                $this->KAL_ID->OldValue = $key;
            }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->iswilayah->setDbValue($row['iswilayah']);
    }
    ...
    public function renderListRow()
    {
        ...
        if ($curVal != "") {
            ...
            if ($this->KEC_ID->ViewValue === null) { // Lookup from database
                $filterWrk = "[KEC_ID]" . SearchString("=", $curVal, DATATYPE_STRING, "");
        ...
        // iswilayah
        $this->iswilayah->ViewValue = $this->iswilayah->CurrentValue;
        $this->iswilayah->ViewCustomAttributes = "";
        ...
        // iswilayah
        $this->iswilayah->LinkCustomAttributes = "";
        $this->iswilayah->HrefValue = "";
        $this->iswilayah->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // iswilayah
        $this->iswilayah->EditAttrs["class"] = "form-control";
        $this->iswilayah->EditCustomAttributes = "";
        if (!$this->iswilayah->Raw) {
            $this->iswilayah->CurrentValue = HtmlDecode($this->iswilayah->CurrentValue);
        }
        $this->iswilayah->EditValue = $this->iswilayah->CurrentValue;
        $this->iswilayah->PlaceHolder = RemoveHtml($this->iswilayah->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->iswilayah);
                } else {
                    $doc->exportCaption($this->KAL_ID);
                    $doc->exportCaption($this->KALURAHAN);
                    $doc->exportCaption($this->KEC_ID);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->iswilayah);
                    } else {
                        $doc->exportField($this->KAL_ID);
                        $doc->exportField($this->KALURAHAN);
                        $doc->exportField($this->KEC_ID);
