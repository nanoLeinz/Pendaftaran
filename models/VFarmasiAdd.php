...
class VFarmasiAdd extends VFarmasi
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->ISRJ->ViewValue = $this->ISRJ->Lookup !== null && is_array($this->ISRJ->Lookup->Options) ? $curVal : null;
            }
            if ($this->ISRJ->ViewValue !== null) { // Load from cache
                $this->ISRJ->EditValue = array_values($this->ISRJ->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[KELUAR_ID]" . SearchString("=", $this->ISRJ->CurrentValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("TREATMENT_OBAT", $detailTblVar) && $detailPage->DetailAdd) {
