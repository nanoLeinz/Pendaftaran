...
/**
 * Table class for CLASS_ROOM_USER
 */
class ClassRoomUser extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'CLASS_ROOM_USER';
        $this->TableName = 'CLASS_ROOM_USER';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[CLASS_ROOM_USER]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // USERNAME
        $this->_USERNAME = new DbField('CLASS_ROOM_USER', 'CLASS_ROOM_USER', 'x__USERNAME', 'USERNAME', '[USERNAME]', '[USERNAME]', 200, 16, -1, false, '[USERNAME]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // ORG_UNIT_CODE
        $this->ORG_UNIT_CODE = new DbField('CLASS_ROOM_USER', 'CLASS_ROOM_USER', 'x_ORG_UNIT_CODE', 'ORG_UNIT_CODE', '[ORG_UNIT_CODE]', '[ORG_UNIT_CODE]', 200, 50, -1, false, '[ORG_UNIT_CODE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // CLASS_ROOM_ID
        $this->CLASS_ROOM_ID = new DbField('CLASS_ROOM_USER', 'CLASS_ROOM_USER', 'x_CLASS_ROOM_ID', 'CLASS_ROOM_ID', '[CLASS_ROOM_ID]', '[CLASS_ROOM_ID]', 200, 16, -1, false, '[CLASS_ROOM_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[CLASS_ROOM_USER]";
    }
    ...
    {
        return "[USERNAME] = '@_USERNAME@' AND [ORG_UNIT_CODE] = '@ORG_UNIT_CODE@' AND [CLASS_ROOM_ID] = '@CLASS_ROOM_ID@'";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 3) {
            ...
            if ($current) {
                $this->CLASS_ROOM_ID->CurrentValue = $keys[2];
            } else {
                $this->CLASS_ROOM_ID->OldValue = $keys[2];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ClassRoomUserList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ClassRoomUserView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ClassRoomUserEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ClassRoomUserAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "ClassRoomUserAdd";
            case Config("API_EDIT_ACTION"):
                return "ClassRoomUserEdit";
            case Config("API_DELETE_ACTION"):
                return "ClassRoomUserDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ClassRoomUserView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ClassRoomUserView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ClassRoomUserAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ClassRoomUserAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClassRoomUserEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ClassRoomUserAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ClassRoomUserDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->CLASS_ROOM_ID->CurrentValue = $key[2];
            } else {
                $this->CLASS_ROOM_ID->OldValue = $key[2];
            }
