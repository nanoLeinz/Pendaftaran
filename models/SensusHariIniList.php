 ...
 * Page class
 */
class SensusHariIniList extends SensusHariIni
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'sensus_hari_ini';

    // Page object name
    public $PageObjName = "SensusHariIniList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fsensus_hari_inilist";
    ...
    public function __construct()
    {
        ...
        // Table object (sensus_hari_ini)
        if (!isset($GLOBALS["sensus_hari_ini"]) || get_class($GLOBALS["sensus_hari_ini"]) == PROJECT_NAMESPACE . "sensus_hari_ini") {
            $GLOBALS["sensus_hari_ini"] = &$this;
        }
        ...
        $this->AddUrl = "SensusHariIniAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "SensusHariIniDelete";
        $this->MultiUpdateUrl = "SensusHariIniUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sensus_hari_ini');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fsensus_hari_inilistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("sensus_hari_ini"));
                $doc->Text = @$content;
    ...
    protected function hideFieldsForAddEdit()
    {
        ...
        if ($this->isAddOrEdit()) {
            $this->NAME_OF_STATUS_PASIEN->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        $this->NAME_OF_STATUS_PASIEN->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->toJson(), ","); // Field NAME_OF_STATUS_PASIEN
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fsensus_hari_inilistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field NAME_OF_STATUS_PASIEN
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->SearchValue = @$filter["x_NAME_OF_STATUS_PASIEN"];
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->SearchOperator = @$filter["z_NAME_OF_STATUS_PASIEN"];
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->SearchCondition = @$filter["v_NAME_OF_STATUS_PASIEN"];
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->SearchValue2 = @$filter["y_NAME_OF_STATUS_PASIEN"];
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->SearchOperator2 = @$filter["w_NAME_OF_STATUS_PASIEN"];
        $this->NAME_OF_STATUS_PASIEN->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->NAME_OF_STATUS_PASIEN, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->NAME_OF_STATUS_PASIEN); // NAME_OF_STATUS_PASIEN
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->NAME_OF_STATUS_PASIEN->setSort("");
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsensus_hari_inilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsensus_hari_inilistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fsensus_hari_inilist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function newRow()
    {
        ...
        $row['NAME_OF_STATUS_PASIEN'] = null;
        return $row;
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsensus_hari_inilistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
