 ...
 * Page class
 */
class ClinicList extends Clinic
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ClinicList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fCLINIClist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "ClinicAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ClinicDelete";
        $this->MultiUpdateUrl = "ClinicUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fCLINIClistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->STYPE_ID->AdvancedSearch->toJson(), ","); // Field STYPE_ID
        ...
        $filterList = Concat($filterList, $this->PROFIT_ID->AdvancedSearch->toJson(), ","); // Field PROFIT_ID
        $filterList = Concat($filterList, $this->SUPPLIED_MM->AdvancedSearch->toJson(), ","); // Field SUPPLIED_MM
        ...
        $filterList = Concat($filterList, $this->PROFILES->AdvancedSearch->toJson(), ","); // Field PROFILES
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fCLINIClistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field STYPE_ID
        $this->STYPE_ID->AdvancedSearch->SearchValue = @$filter["x_STYPE_ID"];
        $this->STYPE_ID->AdvancedSearch->SearchOperator = @$filter["z_STYPE_ID"];
        $this->STYPE_ID->AdvancedSearch->SearchCondition = @$filter["v_STYPE_ID"];
        $this->STYPE_ID->AdvancedSearch->SearchValue2 = @$filter["y_STYPE_ID"];
        $this->STYPE_ID->AdvancedSearch->SearchOperator2 = @$filter["w_STYPE_ID"];
        $this->STYPE_ID->AdvancedSearch->save();
        ...
        // Field PROFIT_ID
        $this->PROFIT_ID->AdvancedSearch->SearchValue = @$filter["x_PROFIT_ID"];
        $this->PROFIT_ID->AdvancedSearch->SearchOperator = @$filter["z_PROFIT_ID"];
        $this->PROFIT_ID->AdvancedSearch->SearchCondition = @$filter["v_PROFIT_ID"];
        $this->PROFIT_ID->AdvancedSearch->SearchValue2 = @$filter["y_PROFIT_ID"];
        $this->PROFIT_ID->AdvancedSearch->SearchOperator2 = @$filter["w_PROFIT_ID"];
        $this->PROFIT_ID->AdvancedSearch->save();

        // Field SUPPLIED_MM
        $this->SUPPLIED_MM->AdvancedSearch->SearchValue = @$filter["x_SUPPLIED_MM"];
        $this->SUPPLIED_MM->AdvancedSearch->SearchOperator = @$filter["z_SUPPLIED_MM"];
        $this->SUPPLIED_MM->AdvancedSearch->SearchCondition = @$filter["v_SUPPLIED_MM"];
        $this->SUPPLIED_MM->AdvancedSearch->SearchValue2 = @$filter["y_SUPPLIED_MM"];
        $this->SUPPLIED_MM->AdvancedSearch->SearchOperator2 = @$filter["w_SUPPLIED_MM"];
        $this->SUPPLIED_MM->AdvancedSearch->save();
        ...
        // Field PROFILES
        $this->PROFILES->AdvancedSearch->SearchValue = @$filter["x_PROFILES"];
        $this->PROFILES->AdvancedSearch->SearchOperator = @$filter["z_PROFILES"];
        $this->PROFILES->AdvancedSearch->SearchCondition = @$filter["v_PROFILES"];
        $this->PROFILES->AdvancedSearch->SearchValue2 = @$filter["y_PROFILES"];
        $this->PROFILES->AdvancedSearch->SearchOperator2 = @$filter["w_PROFILES"];
        $this->PROFILES->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->PROFIT_ID, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SUPPLIED_MM, $arKeywords, $type);
        ...
        $this->buildBasicSearchSql($where, $this->PROFILES, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SPESIALISTIK, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->STYPE_ID); // STYPE_ID
            ...
            $this->updateSort($this->PROFIT_ID); // PROFIT_ID
            $this->updateSort($this->SUPPLIED_MM); // SUPPLIED_MM
            $this->updateSort($this->KDPOLI); // KDPOLI
            $this->updateSort($this->SPESIALISTIK); // SPESIALISTIK
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->ORG_UNIT_CODE->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->CLINIC_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fCLINIClistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fCLINIClistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fCLINIClist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fCLINIClistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
