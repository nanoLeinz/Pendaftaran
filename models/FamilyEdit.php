 ...
 * Page class
 */
class FamilyEdit extends Family
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("NO_REGISTRATION") ?? Key(1) ?? Route(3)) !== null) {
            ...
            if (($keyValue = Get("FAMILY_ID") ?? Key(2) ?? Route(4)) !== null) {
                $this->FAMILY_ID->setQueryStringValue($keyValue);
                $this->FAMILY_ID->setOldValue($this->FAMILY_ID->QueryStringValue);
            } elseif (Post("FAMILY_ID") !== null) {
                $this->FAMILY_ID->setFormValue(Post("FAMILY_ID"));
                $this->FAMILY_ID->setOldValue($this->FAMILY_ID->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_FAMILY_ID")) {
            $this->FAMILY_ID->setOldValue($CurrentForm->getValue("o_FAMILY_ID"));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // FAMILY_ID
            $this->FAMILY_ID->setDbValueDef($rsnew, $this->FAMILY_ID->CurrentValue, 0, $this->FAMILY_ID->ReadOnly);

            // FAMILY_STATUS_ID
            $this->FAMILY_STATUS_ID->setDbValueDef($rsnew, $this->FAMILY_STATUS_ID->CurrentValue, null, $this->FAMILY_STATUS_ID->ReadOnly);

            // NO_REGISTRATION2
            $this->NO_REGISTRATION2->setDbValueDef($rsnew, $this->NO_REGISTRATION2->CurrentValue, null, $this->NO_REGISTRATION2->ReadOnly);

            // FULLNAME
            $this->FULLNAME->setDbValueDef($rsnew, $this->FULLNAME->CurrentValue, null, $this->FULLNAME->ReadOnly);

            // ISRESPONSIBLE
            $this->ISRESPONSIBLE->setDbValueDef($rsnew, $this->ISRESPONSIBLE->CurrentValue, null, $this->ISRESPONSIBLE->ReadOnly);
            ...
            // BLOOD_ID
            $this->BLOOD_ID->setDbValueDef($rsnew, $this->BLOOD_ID->CurrentValue, null, $this->BLOOD_ID->ReadOnly);
