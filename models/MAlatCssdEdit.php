 ...
 * Page class
 */
class MAlatCssdEdit extends MAlatCssd
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MAlatCssdEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("alat_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->alat_id->setQueryStringValue($keyValue);
                $this->alat_id->setOldValue($this->alat_id->QueryStringValue);
            } elseif (Post("alat_id") !== null) {
                $this->alat_id->setFormValue(Post("alat_id"));
                $this->alat_id->setOldValue($this->alat_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->alat_id->IsDetailKey) {
            $this->alat_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->alat_id->CurrentValue = $this->alat_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->alat_id->Required) {
            if (!$this->alat_id->IsDetailKey && EmptyValue($this->alat_id->FormValue)) {
                $this->alat_id->addErrorMessage(str_replace("%s", $this->alat_id->caption(), $this->alat_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // nama_alat
            $this->nama_alat->setDbValueDef($rsnew, $this->nama_alat->CurrentValue, null, $this->nama_alat->ReadOnly);

            // id_set
            $this->id_set->setDbValueDef($rsnew, $this->id_set->CurrentValue, null, $this->id_set->ReadOnly);

            // keadaan
            $this->keadaan->setDbValueDef($rsnew, $this->keadaan->CurrentValue, null, $this->keadaan->ReadOnly);

            // jumlah
            $this->jumlah->setDbValueDef($rsnew, $this->jumlah->CurrentValue, null, $this->jumlah->ReadOnly);

            // merk
            $this->merk->setDbValueDef($rsnew, $this->merk->CurrentValue, null, $this->merk->ReadOnly);
