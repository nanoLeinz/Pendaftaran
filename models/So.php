...
class So extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // MONTH_ID
        $this->MONTH_ID = new DbField('SO', 'SO', 'x_MONTH_ID', 'MONTH_ID', '[MONTH_ID]', 'CAST([MONTH_ID] AS NVARCHAR)', 17, 1, -1, false, '[MONTH_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // START_DATE
        $this->START_DATE = new DbField('SO', 'SO', 'x_START_DATE', 'START_DATE', '[START_DATE]', CastDateFieldForLike("[START_DATE]", 0, "DB"), 135, 8, 0, false, '[START_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // END_DATE
        $this->END_DATE = new DbField('SO', 'SO', 'x_END_DATE', 'END_DATE', '[END_DATE]', CastDateFieldForLike("[END_DATE]", 0, "DB"), 135, 8, 0, false, '[END_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // YEAR_ID
        $this->YEAR_ID = new DbField('SO', 'SO', 'x_YEAR_ID', 'YEAR_ID', '[YEAR_ID]', 'CAST([YEAR_ID] AS NVARCHAR)', 2, 2, -1, false, '[YEAR_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[SO]";
    }
    ...
    {
        return "[MONTH_ID] = @MONTH_ID@ AND [YEAR_ID] = @YEAR_ID@";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SoList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "SoEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SoAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SoView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SoView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SoAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "SoAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("SoEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SoAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SoDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->MONTH_ID->CurrentValue = $key[0];
            } else {
                $this->MONTH_ID->OldValue = $key[0];
            }
            if ($setCurrent) {
