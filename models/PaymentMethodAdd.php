 ...
 * Page class
 */
class PaymentMethodAdd extends PaymentMethod
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PaymentMethodAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (PAYMENT_METHOD)
        if (!isset($GLOBALS["PAYMENT_METHOD"]) || get_class($GLOBALS["PAYMENT_METHOD"]) == PROJECT_NAMESPACE . "PAYMENT_METHOD") {
            $GLOBALS["PAYMENT_METHOD"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'PAYMENT_METHOD');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("PAYMENT_METHOD"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("PAY_METHOD_ID") ?? Route("PAY_METHOD_ID")) !== null) {
                $this->PAY_METHOD_ID->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PaymentMethodList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PaymentMethodList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PaymentMethodView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->PAYMETHOD->CurrentValue = null;
        $this->PAYMETHOD->OldValue = $this->PAYMETHOD->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'PAYMETHOD' first before field var 'x_PAYMETHOD'
        $val = $CurrentForm->hasValue("PAYMETHOD") ? $CurrentForm->getValue("PAYMETHOD") : $CurrentForm->getValue("x_PAYMETHOD");
        if (!$this->PAYMETHOD->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->PAYMETHOD->Visible = false; // Disable update for API request
            } else {
                $this->PAYMETHOD->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->PAYMETHOD->CurrentValue = $this->PAYMETHOD->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['PAYMETHOD'] = $this->PAYMETHOD->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->PAYMETHOD->EditValue = HtmlEncode($this->PAYMETHOD->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->PAYMETHOD->Required) {
            if (!$this->PAYMETHOD->IsDetailKey && EmptyValue($this->PAYMETHOD->FormValue)) {
                $this->PAYMETHOD->addErrorMessage(str_replace("%s", $this->PAYMETHOD->caption(), $this->PAYMETHOD->RequiredErrorMessage));
            }
        }
    ...
    {
        global $Language, $Security;
        if ($this->PAY_METHOD_ID->CurrentValue != "") { // Check field with unique index
            $filter = "([PAY_METHOD_ID] = " . AdjustSql($this->PAY_METHOD_ID->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->PAY_METHOD_ID->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->PAY_METHOD_ID->CurrentValue, $idxErrMsg);
        ...
        // PAYMETHOD
        $this->PAYMETHOD->setDbValueDef($rsnew, $this->PAYMETHOD->CurrentValue, null, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['PAY_METHOD_ID']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PaymentMethodList"), "", $this->TableVar, true);
