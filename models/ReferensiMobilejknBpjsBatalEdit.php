 ...
 * Page class
 */
class ReferensiMobilejknBpjsBatalEdit extends ReferensiMobilejknBpjsBatal
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ReferensiMobilejknBpjsBatalEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("nobooking") ?? Key(0) ?? Route(2)) !== null) {
                $this->nobooking->setQueryStringValue($keyValue);
                $this->nobooking->setOldValue($this->nobooking->QueryStringValue);
            } elseif (Post("nobooking") !== null) {
                $this->nobooking->setFormValue(Post("nobooking"));
                $this->nobooking->setOldValue($this->nobooking->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("nobooking") ?? Route("nobooking")) !== null) {
                    $this->nobooking->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("ReferensiMobilejknBpjsBatalList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "ReferensiMobilejknBpjsBatalList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tanggalbatal' first before field var 'x_tanggalbatal'
        $val = $CurrentForm->hasValue("tanggalbatal") ? $CurrentForm->getValue("tanggalbatal") : $CurrentForm->getValue("x_tanggalbatal");
        if (!$this->tanggalbatal->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tanggalbatal->Visible = false; // Disable update for API request
            } else {
                $this->tanggalbatal->setFormValue($val);
            }
            $this->tanggalbatal->CurrentValue = UnFormatDateTime($this->tanggalbatal->CurrentValue, 0);
        }
        ...
        if ($CurrentForm->hasValue("o_nobooking")) {
            $this->nobooking->setOldValue($CurrentForm->getValue("o_nobooking"));
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tanggalbatal->CurrentValue = $this->tanggalbatal->FormValue;
        $this->tanggalbatal->CurrentValue = UnFormatDateTime($this->tanggalbatal->CurrentValue, 0);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->tanggalbatal->EditValue = HtmlEncode(FormatDateTime($this->tanggalbatal->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if ($this->tanggalbatal->Required) {
            if (!$this->tanggalbatal->IsDetailKey && EmptyValue($this->tanggalbatal->FormValue)) {
                $this->tanggalbatal->addErrorMessage(str_replace("%s", $this->tanggalbatal->caption(), $this->tanggalbatal->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tanggalbatal->FormValue)) {
            $this->tanggalbatal->addErrorMessage($this->tanggalbatal->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // nomorreferensi
            $this->nomorreferensi->setDbValueDef($rsnew, $this->nomorreferensi->CurrentValue, null, $this->nomorreferensi->ReadOnly);

            // tanggalbatal
            $this->tanggalbatal->setDbValueDef($rsnew, UnFormatDateTime($this->tanggalbatal->CurrentValue, 0), null, $this->tanggalbatal->ReadOnly);

            // keterangan
            $this->keterangan->setDbValueDef($rsnew, $this->keterangan->CurrentValue, null, $this->keterangan->ReadOnly);

            // statuskirim
            $this->statuskirim->setDbValueDef($rsnew, $this->statuskirim->CurrentValue, null, $this->statuskirim->ReadOnly);

            // nobooking
            $this->nobooking->setDbValueDef($rsnew, $this->nobooking->CurrentValue, "", $this->nobooking->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ReferensiMobilejknBpjsBatalList"), "", $this->TableVar, true);
        $pageId = "edit";
