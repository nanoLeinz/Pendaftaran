 ...
 * Page class
 */
class ClinicEdit extends Clinic
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("CLINIC_ID") ?? Key(1) ?? Route(3)) !== null) {
                $this->CLINIC_ID->setQueryStringValue($keyValue);
                $this->CLINIC_ID->setOldValue($this->CLINIC_ID->QueryStringValue);
            } elseif (Post("CLINIC_ID") !== null) {
                $this->CLINIC_ID->setFormValue(Post("CLINIC_ID"));
                $this->CLINIC_ID->setOldValue($this->CLINIC_ID->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // CLINIC_ID
            $this->CLINIC_ID->setDbValueDef($rsnew, $this->CLINIC_ID->CurrentValue, "", $this->CLINIC_ID->ReadOnly);

            // NAME_OF_CLINIC
            $this->NAME_OF_CLINIC->setDbValueDef($rsnew, $this->NAME_OF_CLINIC->CurrentValue, null, $this->NAME_OF_CLINIC->ReadOnly);
            ...
            // PROFIT_ID
            $this->PROFIT_ID->setDbValueDef($rsnew, $this->PROFIT_ID->CurrentValue, null, $this->PROFIT_ID->ReadOnly);

            // SUPPLIED_MM
            $this->SUPPLIED_MM->setDbValueDef($rsnew, $this->SUPPLIED_MM->CurrentValue, null, $this->SUPPLIED_MM->ReadOnly);

            // KDPOLI
            $this->KDPOLI->setDbValueDef($rsnew, $this->KDPOLI->CurrentValue, null, $this->KDPOLI->ReadOnly);

            // PICTUREFILE
            $this->PICTUREFILE->setDbValueDef($rsnew, $this->PICTUREFILE->CurrentValue, null, $this->PICTUREFILE->ReadOnly);

            // PROFILES
            $this->PROFILES->setDbValueDef($rsnew, $this->PROFILES->CurrentValue, null, $this->PROFILES->ReadOnly);

            // SPESIALISTIK
            $this->SPESIALISTIK->setDbValueDef($rsnew, $this->SPESIALISTIK->CurrentValue, null, $this->SPESIALISTIK->ReadOnly);
