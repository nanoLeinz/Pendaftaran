 ...
 * Page class
 */
class VLaboratoriumView extends VLaboratorium
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VLaboratoriumView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("IDXDAFTAR") ?? Route("IDXDAFTAR")) !== null) {
                ...
                $returnUrl = "VLaboratoriumList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "VLaboratoriumList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "VLaboratoriumList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("TreatmentBillList?" . Config("TABLE_SHOW_MASTER") . "=V_LABORATORIUM&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("TreatmentBillGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_LABORATORIUM')) {
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_LABORATORIUM')) {
