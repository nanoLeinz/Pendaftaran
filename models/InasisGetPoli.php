...
/**
 * Table class for INASIS_GET_POLI
 */
class InasisGetPoli extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'INASIS_GET_POLI';
        $this->TableName = 'INASIS_GET_POLI';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[INASIS_GET_POLI]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // KDPOLI
        $this->KDPOLI = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_KDPOLI', 'KDPOLI', '[KDPOLI]', '[KDPOLI]', 129, 3, -1, false, '[KDPOLI]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->KDPOLI->IsPrimaryKey = true; // Primary key field
        $this->KDPOLI->Nullable = false; // NOT NULL field
        $this->KDPOLI->Required = true; // Required field
        ...
        // NMPOLI
        $this->NMPOLI = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_NMPOLI', 'NMPOLI', '[NMPOLI]', '[NMPOLI]', 200, 100, -1, false, '[NMPOLI]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // REST_CODE
        $this->REST_CODE = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_REST_CODE', 'REST_CODE', '[REST_CODE]', '[REST_CODE]', 129, 3, -1, false, '[REST_CODE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // REST_MESSAGE
        $this->REST_MESSAGE = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_REST_MESSAGE', 'REST_MESSAGE', '[REST_MESSAGE]', '[REST_MESSAGE]', 200, 50, -1, false, '[REST_MESSAGE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // REST_DATE
        $this->REST_DATE = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_REST_DATE', 'REST_DATE', '[REST_DATE]', CastDateFieldForLike("[REST_DATE]", 0, "DB"), 135, 8, 0, false, '[REST_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // REST_METHOD
        $this->REST_METHOD = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_REST_METHOD', 'REST_METHOD', '[REST_METHOD]', '[REST_METHOD]', 200, 10, -1, false, '[REST_METHOD]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MODIFIED_DATE
        $this->MODIFIED_DATE = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_MODIFIED_DATE', 'MODIFIED_DATE', '[MODIFIED_DATE]', CastDateFieldForLike("[MODIFIED_DATE]", 0, "DB"), 135, 8, 0, false, '[MODIFIED_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MODIFIED_BY
        $this->MODIFIED_BY = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_MODIFIED_BY', 'MODIFIED_BY', '[MODIFIED_BY]', '[MODIFIED_BY]', 200, 50, -1, false, '[MODIFIED_BY]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->MODIFIED_BY->Sortable = true; // Allow sort
        $this->MODIFIED_BY->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->MODIFIED_BY->Param, "CustomMsg");
        $this->Fields['MODIFIED_BY'] = &$this->MODIFIED_BY;

        // RESPON
        $this->RESPON = new DbField('INASIS_GET_POLI', 'INASIS_GET_POLI', 'x_RESPON', 'RESPON', '[RESPON]', '[RESPON]', 201, 0, -1, false, '[RESPON]', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[INASIS_GET_POLI]";
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('KDPOLI', $rs)) {
                AddFilter($where, QuotedName('KDPOLI', $this->Dbid) . '=' . QuotedValue($rs['KDPOLI'], $this->KDPOLI->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->KDPOLI->CurrentValue : $this->KDPOLI->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->KDPOLI->CurrentValue = $keys[0];
            } else {
                $this->KDPOLI->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('KDPOLI', $row) ? $row['KDPOLI'] : null;
        } else {
            $val = $this->KDPOLI->OldValue !== null ? $this->KDPOLI->OldValue : $this->KDPOLI->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@KDPOLI@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InasisGetPoliList");
    }
    ...
    {
        global $Language;
        if ($pageName == "InasisGetPoliView") {
            return $Language->phrase("View");
        } elseif ($pageName == "InasisGetPoliEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "InasisGetPoliAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "InasisGetPoliView";
            case Config("API_ADD_ACTION"):
                return "InasisGetPoliAdd";
            case Config("API_EDIT_ACTION"):
                return "InasisGetPoliEdit";
            case Config("API_DELETE_ACTION"):
                return "InasisGetPoliDelete";
            case Config("API_LIST_ACTION"):
                return "InasisGetPoliList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InasisGetPoliView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("InasisGetPoliView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "InasisGetPoliAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "InasisGetPoliAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InasisGetPoliEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InasisGetPoliAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("InasisGetPoliDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "KDPOLI:" . JsonEncode($this->KDPOLI->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->KDPOLI->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->KDPOLI->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("KDPOLI") ?? Route("KDPOLI")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->KDPOLI->CurrentValue = $key;
            } else {
                $this->KDPOLI->OldValue = $key;
            }
