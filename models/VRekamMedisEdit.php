 ...
 * Page class
 */
class VRekamMedisEdit extends VRekamMedis
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VRekamMedisEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("VRekamMedisList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->GetViewUrl();
                if (GetPageName($returnUrl) == "VRekamMedisList") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'CETAK_DOC' first before field var 'x_CETAK_DOC'
        $val = $CurrentForm->hasValue("CETAK_DOC") ? $CurrentForm->getValue("CETAK_DOC") : $CurrentForm->getValue("x_CETAK_DOC");
        if (!$this->CETAK_DOC->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->CETAK_DOC->Visible = false; // Disable update for API request
            } else {
                $this->CETAK_DOC->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->CETAK_DOC->CurrentValue = $this->CETAK_DOC->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->CETAK_DOC->EditValue = HtmlEncode($this->CETAK_DOC->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->CETAK_DOC->Required) {
            if (!$this->CETAK_DOC->IsDetailKey && EmptyValue($this->CETAK_DOC->FormValue)) {
                $this->CETAK_DOC->addErrorMessage(str_replace("%s", $this->CETAK_DOC->caption(), $this->CETAK_DOC->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // BOOKED_DATE
            $this->BOOKED_DATE->CurrentValue = CurrentDate();
            $this->BOOKED_DATE->setDbValueDef($rsnew, $this->BOOKED_DATE->CurrentValue, null);

            // VISIT_DATE
            $this->VISIT_DATE->CurrentValue = CurrentDate();
            ...
            // CETAK_DOC
            $this->CETAK_DOC->setDbValueDef($rsnew, $this->CETAK_DOC->CurrentValue, "", $this->CETAK_DOC->ReadOnly);
