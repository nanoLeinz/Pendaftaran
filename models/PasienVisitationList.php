 ...
 * Page class
 */
class PasienVisitationList extends PasienVisitation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PasienVisitationList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fPASIEN_VISITATIONlist";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "PasienVisitationDelete";
        $this->MultiUpdateUrl = "PasienVisitationUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fPASIEN_VISITATIONlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->Faskes->AdvancedSearch->toJson(), ","); // Field Faskes
        $filterList = Concat($filterList, $this->SEP->AdvancedSearch->toJson(), ","); // Field SEP
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fPASIEN_VISITATIONlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Faskes
        $this->Faskes->AdvancedSearch->SearchValue = @$filter["x_Faskes"];
        $this->Faskes->AdvancedSearch->SearchOperator = @$filter["z_Faskes"];
        $this->Faskes->AdvancedSearch->SearchCondition = @$filter["v_Faskes"];
        $this->Faskes->AdvancedSearch->SearchValue2 = @$filter["y_Faskes"];
        $this->Faskes->AdvancedSearch->SearchOperator2 = @$filter["w_Faskes"];
        $this->Faskes->AdvancedSearch->save();

        // Field SEP
        $this->SEP->AdvancedSearch->SearchValue = @$filter["x_SEP"];
        $this->SEP->AdvancedSearch->SearchOperator = @$filter["z_SEP"];
        $this->SEP->AdvancedSearch->SearchCondition = @$filter["v_SEP"];
        $this->SEP->AdvancedSearch->SearchValue2 = @$filter["y_SEP"];
        $this->SEP->AdvancedSearch->SearchOperator2 = @$filter["w_SEP"];
        $this->SEP->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Faskes, $default, false); // Faskes
        $this->buildSearchSql($where, $this->SEP, $default, false); // SEP
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->Faskes->AdvancedSearch->save(); // Faskes
            $this->SEP->AdvancedSearch->save(); // SEP
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->Faskes, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->SEP, $arKeywords, $type);
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Faskes->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->SEP->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->Faskes->AdvancedSearch->unsetSession();
                $this->SEP->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->Faskes->AdvancedSearch->load();
                $this->SEP->AdvancedSearch->load();
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fPASIEN_VISITATIONlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fPASIEN_VISITATIONlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fPASIEN_VISITATIONlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // Faskes
        if (!$this->isAddOrEdit() && $this->Faskes->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Faskes->AdvancedSearch->SearchValue != "" || $this->Faskes->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // SEP
        if (!$this->isAddOrEdit() && $this->SEP->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->SEP->AdvancedSearch->SearchValue != "" || $this->SEP->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->SEP->Raw) {
                $this->SEP->AdvancedSearch->SearchValue = HtmlDecode($this->SEP->AdvancedSearch->SearchValue);
            }
            $this->SEP->EditValue = HtmlEncode($this->SEP->AdvancedSearch->SearchValue);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fPASIEN_VISITATIONlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"PasienVisitationSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
