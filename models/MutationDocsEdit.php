...
class MutationDocsEdit extends MutationDocs
{
    ...
    public function run()
    {
        ...
        $this->CLINIC_ID_TO->Visible = false;
        $this->MUTATION_DATE->Visible = false;
        $this->MUTATION_BY->Visible = false;
        $this->MUTATION_VALUE->Visible = false;
        $this->ORDER_VALUE->Visible = false;
        $this->YEAR_ID->Visible = false;
        $this->RECEIVED_BY->Visible = false;
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("DOC_NO") ?? Key(1) ?? Route(3)) !== null) {
                $this->DOC_NO->setQueryStringValue($keyValue);
                $this->DOC_NO->setOldValue($this->DOC_NO->QueryStringValue);
            } elseif (Post("DOC_NO") !== null) {
                $this->DOC_NO->setFormValue(Post("DOC_NO"));
                $this->DOC_NO->setOldValue($this->DOC_NO->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
