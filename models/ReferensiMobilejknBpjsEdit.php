 ...
 * Page class
 */
class ReferensiMobilejknBpjsEdit extends ReferensiMobilejknBpjs
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ReferensiMobilejknBpjsEdit";
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // nobooking
            $this->nobooking->setDbValueDef($rsnew, $this->nobooking->CurrentValue, null, $this->nobooking->ReadOnly);

            // no_rawat
            $this->no_rawat->setDbValueDef($rsnew, $this->no_rawat->CurrentValue, null, $this->no_rawat->ReadOnly);

            // nomorkartu
            $this->nomorkartu->setDbValueDef($rsnew, $this->nomorkartu->CurrentValue, null, $this->nomorkartu->ReadOnly);

            // nik
            $this->nik->setDbValueDef($rsnew, $this->nik->CurrentValue, null, $this->nik->ReadOnly);

            // nohp
            $this->nohp->setDbValueDef($rsnew, $this->nohp->CurrentValue, null, $this->nohp->ReadOnly);

            // kodepoli
            $this->kodepoli->setDbValueDef($rsnew, $this->kodepoli->CurrentValue, null, $this->kodepoli->ReadOnly);

            // pasienbaru
            $this->pasienbaru->setDbValueDef($rsnew, $this->pasienbaru->CurrentValue, 0, $this->pasienbaru->ReadOnly);

            // norm
            $this->norm->setDbValueDef($rsnew, $this->norm->CurrentValue, null, $this->norm->ReadOnly);

            // tanggalperiksa
            $this->tanggalperiksa->setDbValueDef($rsnew, UnFormatDateTime($this->tanggalperiksa->CurrentValue, 0), null, $this->tanggalperiksa->ReadOnly);

            // kodedokter
            $this->kodedokter->setDbValueDef($rsnew, $this->kodedokter->CurrentValue, null, $this->kodedokter->ReadOnly);

            // jampraktek
            $this->jampraktek->setDbValueDef($rsnew, $this->jampraktek->CurrentValue, null, $this->jampraktek->ReadOnly);

            // jeniskunjungan
            $this->jeniskunjungan->setDbValueDef($rsnew, $this->jeniskunjungan->CurrentValue, null, $this->jeniskunjungan->ReadOnly);

            // nomorreferensi
            $this->nomorreferensi->setDbValueDef($rsnew, $this->nomorreferensi->CurrentValue, "", $this->nomorreferensi->ReadOnly);

            // nomorantrean
            $this->nomorantrean->setDbValueDef($rsnew, $this->nomorantrean->CurrentValue, null, $this->nomorantrean->ReadOnly);

            // angkaantrean
            $this->angkaantrean->setDbValueDef($rsnew, $this->angkaantrean->CurrentValue, null, $this->angkaantrean->ReadOnly);

            // estimasidilayani
            $this->estimasidilayani->setDbValueDef($rsnew, $this->estimasidilayani->CurrentValue, null, $this->estimasidilayani->ReadOnly);

            // sisakuotajkn
            $this->sisakuotajkn->setDbValueDef($rsnew, $this->sisakuotajkn->CurrentValue, null, $this->sisakuotajkn->ReadOnly);

            // kuotajkn
            $this->kuotajkn->setDbValueDef($rsnew, $this->kuotajkn->CurrentValue, null, $this->kuotajkn->ReadOnly);

            // sisakuotanonjkn
            $this->sisakuotanonjkn->setDbValueDef($rsnew, $this->sisakuotanonjkn->CurrentValue, null, $this->sisakuotanonjkn->ReadOnly);

            // kuotanonjkn
            $this->kuotanonjkn->setDbValueDef($rsnew, $this->kuotanonjkn->CurrentValue, null, $this->kuotanonjkn->ReadOnly);

            // status
            $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, null, $this->status->ReadOnly);

            // validasi
            $this->validasi->setDbValueDef($rsnew, UnFormatDateTime($this->validasi->CurrentValue, 0), null, $this->validasi->ReadOnly);
