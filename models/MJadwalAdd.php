 ...
 * Page class
 */
class MJadwalAdd extends MJadwal
{
    use MessagesTrait;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("MJadwalList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "MJadwalList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "MJadwalView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->hari_kerja->CurrentValue = null;
        $this->hari_kerja->OldValue = $this->hari_kerja->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'hari_kerja' first before field var 'x_hari_kerja'
        $val = $CurrentForm->hasValue("hari_kerja") ? $CurrentForm->getValue("hari_kerja") : $CurrentForm->getValue("x_hari_kerja");
        if (!$this->hari_kerja->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->hari_kerja->Visible = false; // Disable update for API request
            } else {
                $this->hari_kerja->setFormValue($val);
            }
        }
        ...
        // Check field name 'kouta' first before field var 'x_kouta'
        $val = $CurrentForm->hasValue("kouta") ? $CurrentForm->getValue("kouta") : $CurrentForm->getValue("x_kouta");
        if (!$this->kouta->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->kouta->Visible = false; // Disable update for API request
            } else {
                $this->kouta->setFormValue($val);
            }
        }

        // Check field name 'id_jadwal' first before field var 'x_id_jadwal'
        $val = $CurrentForm->hasValue("id_jadwal") ? $CurrentForm->getValue("id_jadwal") : $CurrentForm->getValue("x_id_jadwal");
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->hari_kerja->CurrentValue = $this->hari_kerja->FormValue;
        ...
        $this->kouta->CurrentValue = $this->kouta->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['hari_kerja'] = $this->hari_kerja->CurrentValue;
        ...
        $row['kouta'] = $this->kouta->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->hari_kerja->EditValue = HtmlEncode($this->hari_kerja->CurrentValue);
            ...
            $this->kouta->EditValue = HtmlEncode($this->kouta->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->hari_kerja->Required) {
            if (!$this->hari_kerja->IsDetailKey && EmptyValue($this->hari_kerja->FormValue)) {
                $this->hari_kerja->addErrorMessage(str_replace("%s", $this->hari_kerja->caption(), $this->hari_kerja->RequiredErrorMessage));
            }
        }
        ...
        if ($this->kouta->Required) {
            if (!$this->kouta->IsDetailKey && EmptyValue($this->kouta->FormValue)) {
                $this->kouta->addErrorMessage(str_replace("%s", $this->kouta->caption(), $this->kouta->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->kouta->FormValue)) {
            $this->kouta->addErrorMessage($this->kouta->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // hari_kerja
        $this->hari_kerja->setDbValueDef($rsnew, $this->hari_kerja->CurrentValue, null, false);
        ...
        // kouta
        $this->kouta->setDbValueDef($rsnew, $this->kouta->CurrentValue, null, strval($this->kouta->CurrentValue) == "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("MJadwalList"), "", $this->TableVar, true);
