...
/**
 * Table class for OPERATION_TASK
 */
class OperationTask extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'OPERATION_TASK';
        $this->TableName = 'OPERATION_TASK';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[OPERATION_TASK]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // ORG_UNIT_CODE
        $this->ORG_UNIT_CODE = new DbField('OPERATION_TASK', 'OPERATION_TASK', 'x_ORG_UNIT_CODE', 'ORG_UNIT_CODE', '[ORG_UNIT_CODE]', '[ORG_UNIT_CODE]', 200, 50, -1, false, '[ORG_UNIT_CODE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // TASK_ID
        $this->TASK_ID = new DbField('OPERATION_TASK', 'OPERATION_TASK', 'x_TASK_ID', 'TASK_ID', '[TASK_ID]', 'CAST([TASK_ID] AS NVARCHAR)', 17, 1, -1, false, '[TASK_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // TARIF_ID
        $this->TARIF_ID = new DbField('OPERATION_TASK', 'OPERATION_TASK', 'x_TARIF_ID', 'TARIF_ID', '[TARIF_ID]', '[TARIF_ID]', 200, 50, -1, false, '[TARIF_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // TASK
        $this->TASK = new DbField('OPERATION_TASK', 'OPERATION_TASK', 'x_TASK', 'TASK', '[TASK]', '[TASK]', 200, 100, -1, false, '[TASK]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->TASK->Sortable = true; // Allow sort
        $this->TASK->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->TASK->Param, "CustomMsg");
        $this->Fields['TASK'] = &$this->TASK;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[OPERATION_TASK]";
    }
    ...
    {
        return "[ORG_UNIT_CODE] = '@ORG_UNIT_CODE@' AND [TASK_ID] = @TASK_ID@";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("OperationTaskList");
    }
    ...
    {
        global $Language;
        if ($pageName == "OperationTaskView") {
            return $Language->phrase("View");
        } elseif ($pageName == "OperationTaskEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "OperationTaskAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "OperationTaskAdd";
            case Config("API_EDIT_ACTION"):
                return "OperationTaskEdit";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("OperationTaskView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("OperationTaskView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "OperationTaskAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "OperationTaskAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OperationTaskEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OperationTaskAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("OperationTaskDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        // TASK
        $this->TASK->ViewValue = $this->TASK->CurrentValue;
        $this->TASK->ViewCustomAttributes = "";
        ...
        // TASK
        $this->TASK->LinkCustomAttributes = "";
        $this->TASK->HrefValue = "";
        $this->TASK->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // TASK
        $this->TASK->EditAttrs["class"] = "form-control";
        $this->TASK->EditCustomAttributes = "";
        if (!$this->TASK->Raw) {
            $this->TASK->CurrentValue = HtmlDecode($this->TASK->CurrentValue);
        }
        $this->TASK->EditValue = $this->TASK->CurrentValue;
        $this->TASK->PlaceHolder = RemoveHtml($this->TASK->caption());
