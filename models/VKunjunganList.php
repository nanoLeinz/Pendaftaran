 ...
 * Page class
 */
class VKunjunganList extends VKunjungan
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'V_KUNJUNGAN';

    // Page object name
    public $PageObjName = "VKunjunganList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_KUNJUNGANlist";
    ...
    public function __construct()
    {
        ...
        // Table object (V_KUNJUNGAN)
        if (!isset($GLOBALS["V_KUNJUNGAN"]) || get_class($GLOBALS["V_KUNJUNGAN"]) == PROJECT_NAMESPACE . "V_KUNJUNGAN") {
            $GLOBALS["V_KUNJUNGAN"] = &$this;
        }
        ...
        $this->AddUrl = "VKunjunganAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VKunjunganDelete";
        $this->MultiUpdateUrl = "VKunjunganUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'V_KUNJUNGAN');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fV_KUNJUNGANlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("V_KUNJUNGAN"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fV_KUNJUNGANlistsrch", $filters);
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "dbo.PASIEN_VISITATION.VISIT_DATE DESC";
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->NO_REGISTRATION->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->VISIT_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fV_KUNJUNGANlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fV_KUNJUNGANlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fV_KUNJUNGANlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->MONTH->AdvancedSearch->SearchValue));
            if ($curVal != "") {
            ...
            $curVal = trim(strval($this->YEAR->AdvancedSearch->SearchValue));
            if ($curVal != "") {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fV_KUNJUNGANlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
