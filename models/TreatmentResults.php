...
/**
 * Table class for TREATMENT_RESULTS
 */
class TreatmentResults extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'TREATMENT_RESULTS';
        $this->TableName = 'TREATMENT_RESULTS';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[TREATMENT_RESULTS]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // RESULT_ID
        $this->RESULT_ID = new DbField('TREATMENT_RESULTS', 'TREATMENT_RESULTS', 'x_RESULT_ID', 'RESULT_ID', '[RESULT_ID]', 'CAST([RESULT_ID] AS NVARCHAR)', 17, 1, -1, false, '[RESULT_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // RESULTS
        $this->RESULTS = new DbField('TREATMENT_RESULTS', 'TREATMENT_RESULTS', 'x_RESULTS', 'RESULTS', '[RESULTS]', '[RESULTS]', 200, 100, -1, false, '[RESULTS]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // DESCRIPTION
        $this->DESCRIPTION = new DbField('TREATMENT_RESULTS', 'TREATMENT_RESULTS', 'x_DESCRIPTION', 'DESCRIPTION', '[DESCRIPTION]', '[DESCRIPTION]', 200, 100, -1, false, '[DESCRIPTION]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[TREATMENT_RESULTS]";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->RESULT_ID->CurrentValue = $keys[0];
            } else {
                $this->RESULT_ID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TreatmentResultsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "TreatmentResultsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "TreatmentResultsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "TreatmentResultsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "TreatmentResultsView";
            case Config("API_ADD_ACTION"):
                return "TreatmentResultsAdd";
            case Config("API_EDIT_ACTION"):
                return "TreatmentResultsEdit";
            case Config("API_DELETE_ACTION"):
                return "TreatmentResultsDelete";
            case Config("API_LIST_ACTION"):
                return "TreatmentResultsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TreatmentResultsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("TreatmentResultsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "TreatmentResultsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "TreatmentResultsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TreatmentResultsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TreatmentResultsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("TreatmentResultsDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "RESULT_ID:" . JsonEncode($this->RESULT_ID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->RESULT_ID->CurrentValue = $key;
            } else {
                $this->RESULT_ID->OldValue = $key;
            }
