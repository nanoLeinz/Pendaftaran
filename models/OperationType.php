...
/**
 * Table class for OPERATION_TYPE
 */
class OperationType extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'OPERATION_TYPE';
        $this->TableName = 'OPERATION_TYPE';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[OPERATION_TYPE]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // OPERATION_TYPE
        $this->OPERATION_TYPE = new DbField('OPERATION_TYPE', 'OPERATION_TYPE', 'x_OPERATION_TYPE', 'OPERATION_TYPE', '[OPERATION_TYPE]', '[OPERATION_TYPE]', 200, 10, -1, false, '[OPERATION_TYPE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->OPERATION_TYPE->IsPrimaryKey = true; // Primary key field
        $this->OPERATION_TYPE->Nullable = false; // NOT NULL field
        $this->OPERATION_TYPE->Required = true; // Required field
        ...
        // OPERATION
        $this->OPERATION = new DbField('OPERATION_TYPE', 'OPERATION_TYPE', 'x_OPERATION', 'OPERATION', '[OPERATION]', '[OPERATION]', 200, 100, -1, false, '[OPERATION]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->OPERATION->Sortable = true; // Allow sort
        $this->OPERATION->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->OPERATION->Param, "CustomMsg");
        $this->Fields['OPERATION'] = &$this->OPERATION;

        // SPECIALIST_TYPE_ID
        $this->SPECIALIST_TYPE_ID = new DbField('OPERATION_TYPE', 'OPERATION_TYPE', 'x_SPECIALIST_TYPE_ID', 'SPECIALIST_TYPE_ID', '[SPECIALIST_TYPE_ID]', '[SPECIALIST_TYPE_ID]', 200, 10, -1, false, '[SPECIALIST_TYPE_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[OPERATION_TYPE]";
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('OPERATION_TYPE', $rs)) {
                AddFilter($where, QuotedName('OPERATION_TYPE', $this->Dbid) . '=' . QuotedValue($rs['OPERATION_TYPE'], $this->OPERATION_TYPE->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->OPERATION_TYPE->CurrentValue : $this->OPERATION_TYPE->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->OPERATION_TYPE->CurrentValue = $keys[0];
            } else {
                $this->OPERATION_TYPE->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('OPERATION_TYPE', $row) ? $row['OPERATION_TYPE'] : null;
        } else {
            $val = $this->OPERATION_TYPE->OldValue !== null ? $this->OPERATION_TYPE->OldValue : $this->OPERATION_TYPE->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@OPERATION_TYPE@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("OperationTypeList");
    }
    ...
    {
        global $Language;
        if ($pageName == "OperationTypeView") {
            return $Language->phrase("View");
        } elseif ($pageName == "OperationTypeEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "OperationTypeAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("OperationTypeView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("OperationTypeView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "OperationTypeAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "OperationTypeAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OperationTypeEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OperationTypeAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("OperationTypeDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "OPERATION_TYPE:" . JsonEncode($this->OPERATION_TYPE->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->OPERATION_TYPE->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->OPERATION_TYPE->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("OPERATION_TYPE") ?? Route("OPERATION_TYPE")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->OPERATION_TYPE->CurrentValue = $key;
            } else {
                $this->OPERATION_TYPE->OldValue = $key;
            }
