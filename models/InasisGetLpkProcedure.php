...
/**
 * Table class for INASIS_GET_LPK_PROCEDURE
 */
class InasisGetLpkProcedure extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'INASIS_GET_LPK_PROCEDURE';
        $this->TableName = 'INASIS_GET_LPK_PROCEDURE';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[INASIS_GET_LPK_PROCEDURE]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // LPK_ID
        $this->LPK_ID = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_LPK_ID', 'LPK_ID', '[LPK_ID]', '[LPK_ID]', 200, 50, -1, false, '[LPK_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // NOSEP
        $this->NOSEP = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_NOSEP', 'NOSEP', '[NOSEP]', '[NOSEP]', 200, 50, -1, false, '[NOSEP]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // NOKARTU
        $this->NOKARTU = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_NOKARTU', 'NOKARTU', '[NOKARTU]', '[NOKARTU]', 200, 50, -1, false, '[NOKARTU]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // KDDIAG
        $this->KDDIAG = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_KDDIAG', 'KDDIAG', '[KDDIAG]', '[KDDIAG]', 200, 50, -1, false, '[KDDIAG]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->KDDIAG->IsPrimaryKey = true; // Primary key field
        $this->KDDIAG->Nullable = false; // NOT NULL field
        $this->KDDIAG->Required = true; // Required field
        ...
        // NMDIAG
        $this->NMDIAG = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_NMDIAG', 'NMDIAG', '[NMDIAG]', '[NMDIAG]', 200, 250, -1, false, '[NMDIAG]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MODIFIED_DATE
        $this->MODIFIED_DATE = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_MODIFIED_DATE', 'MODIFIED_DATE', '[MODIFIED_DATE]', CastDateFieldForLike("[MODIFIED_DATE]", 0, "DB"), 135, 8, 0, false, '[MODIFIED_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MODIFIED_BY
        $this->MODIFIED_BY = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_MODIFIED_BY', 'MODIFIED_BY', '[MODIFIED_BY]', '[MODIFIED_BY]', 130, 10, -1, false, '[MODIFIED_BY]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->MODIFIED_BY->Sortable = true; // Allow sort
        $this->MODIFIED_BY->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->MODIFIED_BY->Param, "CustomMsg");
        $this->Fields['MODIFIED_BY'] = &$this->MODIFIED_BY;

        // RESPON
        $this->RESPON = new DbField('INASIS_GET_LPK_PROCEDURE', 'INASIS_GET_LPK_PROCEDURE', 'x_RESPON', 'RESPON', '[RESPON]', '[RESPON]', 201, 0, -1, false, '[RESPON]', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[INASIS_GET_LPK_PROCEDURE]";
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            ...
            if (array_key_exists('KDDIAG', $rs)) {
                AddFilter($where, QuotedName('KDDIAG', $this->Dbid) . '=' . QuotedValue($rs['KDDIAG'], $this->KDDIAG->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "[LPK_ID] = '@LPK_ID@' AND [NOSEP] = '@NOSEP@' AND [NOKARTU] = '@NOKARTU@' AND [KDDIAG] = '@KDDIAG@'";
    }
    ...
    public function getKey($current = false)
    {
        ...
        $val = $current ? $this->KDDIAG->CurrentValue : $this->KDDIAG->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 4) {
            ...
            if ($current) {
                $this->NOKARTU->CurrentValue = $keys[2];
            } else {
                $this->NOKARTU->OldValue = $keys[2];
            }
            if ($current) {
                $this->KDDIAG->CurrentValue = $keys[3];
            } else {
                $this->KDDIAG->OldValue = $keys[3];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('KDDIAG', $row) ? $row['KDDIAG'] : null;
        } else {
            $val = $this->KDDIAG->OldValue !== null ? $this->KDDIAG->OldValue : $this->KDDIAG->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@KDDIAG@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InasisGetLpkProcedureList");
    }
    ...
    {
        global $Language;
        if ($pageName == "InasisGetLpkProcedureView") {
            return $Language->phrase("View");
        } elseif ($pageName == "InasisGetLpkProcedureEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "InasisGetLpkProcedureAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "InasisGetLpkProcedureView";
            case Config("API_ADD_ACTION"):
                return "InasisGetLpkProcedureAdd";
            case Config("API_EDIT_ACTION"):
                return "InasisGetLpkProcedureEdit";
            case Config("API_DELETE_ACTION"):
                return "InasisGetLpkProcedureDelete";
            case Config("API_LIST_ACTION"):
                return "InasisGetLpkProcedureList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InasisGetLpkProcedureView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("InasisGetLpkProcedureView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "InasisGetLpkProcedureAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "InasisGetLpkProcedureAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InasisGetLpkProcedureEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InasisGetLpkProcedureAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("InasisGetLpkProcedureDelete", $this->getUrlParm());
    }
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",KDDIAG:" . JsonEncode($this->KDDIAG->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->KDDIAG->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->KDDIAG->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("KDDIAG") ?? Route("KDDIAG")) !== null) {
                $arKey[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->NOKARTU->CurrentValue = $key[2];
            } else {
                $this->NOKARTU->OldValue = $key[2];
            }
            if ($setCurrent) {
                $this->KDDIAG->CurrentValue = $key[3];
            } else {
                $this->KDDIAG->OldValue = $key[3];
            }
