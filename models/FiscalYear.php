...
/**
 * Table class for FISCAL_YEAR
 */
class FiscalYear extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'FISCAL_YEAR';
        $this->TableName = 'FISCAL_YEAR';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[FISCAL_YEAR]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // YEAR_ID
        $this->YEAR_ID = new DbField('FISCAL_YEAR', 'FISCAL_YEAR', 'x_YEAR_ID', 'YEAR_ID', '[YEAR_ID]', '[YEAR_ID]', 200, 4, -1, false, '[YEAR_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // START_DATE
        $this->START_DATE = new DbField('FISCAL_YEAR', 'FISCAL_YEAR', 'x_START_DATE', 'START_DATE', '[START_DATE]', CastDateFieldForLike("[START_DATE]", 0, "DB"), 135, 8, 0, false, '[START_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // END_DATE
        $this->END_DATE = new DbField('FISCAL_YEAR', 'FISCAL_YEAR', 'x_END_DATE', 'END_DATE', '[END_DATE]', CastDateFieldForLike("[END_DATE]", 0, "DB"), 135, 8, 0, false, '[END_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[FISCAL_YEAR]";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FiscalYearList");
    }
    ...
    {
        global $Language;
        if ($pageName == "FiscalYearView") {
            return $Language->phrase("View");
        } elseif ($pageName == "FiscalYearEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FiscalYearAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "FiscalYearAdd";
            case Config("API_EDIT_ACTION"):
                return "FiscalYearEdit";
            case Config("API_DELETE_ACTION"):
                return "FiscalYearDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FiscalYearView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FiscalYearView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FiscalYearAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FiscalYearAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("FiscalYearEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FiscalYearAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FiscalYearDelete", $this->getUrlParm());
    }
    ...
    {
        $json = "";
        $json .= "YEAR_ID:" . JsonEncode($this->YEAR_ID->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->YEAR_ID->CurrentValue = $key;
            } else {
                $this->YEAR_ID->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->YEAR_ID->Raw) {
            $this->YEAR_ID->CurrentValue = HtmlDecode($this->YEAR_ID->CurrentValue);
        }
