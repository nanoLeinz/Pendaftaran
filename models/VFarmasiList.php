 ...
 * Page class
 */
class VFarmasiList extends VFarmasi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VFarmasiList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_FARMASIlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "VFarmasiAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VFarmasiDelete";
        $this->MultiUpdateUrl = "VFarmasiUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fV_FARMASIlistsrch";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fV_FARMASIlistsrch", $filters);
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->COB, $default, true); // COB
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_TREATMENT_OBAT"
        $item = &$this->ListOptions->add("detail_TREATMENT_OBAT");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'TREATMENT_OBAT') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_TREATMENT_OBAT"
        $opt = $this->ListOptions["detail_TREATMENT_OBAT"];
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_OBAT')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("TREATMENT_OBAT", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentObatList?" . Config("TABLE_SHOW_MASTER") . "=V_FARMASI&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("TreatmentObatGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_FARMASI')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_OBAT");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "TREATMENT_OBAT";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_FARMASI')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_OBAT");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "TREATMENT_OBAT";
            }
            if ($links != "") {
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_BILL')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentBillList?" . Config("TABLE_SHOW_MASTER") . "=V_FARMASI&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_TREATMENT_OBAT");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_OBAT");
                $detailPage = Container("TreatmentObatGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'V_FARMASI') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "TREATMENT_OBAT";
                }
                $item = &$option->add("detailadd_TREATMENT_BILL");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=TREATMENT_BILL");
                $detailPage = Container("TreatmentBillGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'V_FARMASI') && $Security->canAdd());
                if ($item->Visible) {
                    ...
                    $detailTableLink .= "TREATMENT_BILL";
                }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fV_FARMASIlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fV_FARMASIlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fV_FARMASIlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fV_FARMASIlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"VFarmasiSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
