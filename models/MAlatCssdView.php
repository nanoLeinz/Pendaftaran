 ...
 * Page class
 */
class MAlatCssdView extends MAlatCssd
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MAlatCssdView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("alat_id") ?? Route("alat_id")) !== null) {
                ...
                $this->RecKey["alat_id"] = $this->alat_id->QueryStringValue;
            } elseif (Post("alat_id") !== null) {
                $this->alat_id->setFormValue(Post("alat_id"));
                $this->RecKey["alat_id"] = $this->alat_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->alat_id->setQueryStringValue($keyValue);
                $this->RecKey["alat_id"] = $this->alat_id->QueryStringValue;
            } else {
                $returnUrl = "MAlatCssdList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "MAlatCssdList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "MAlatCssdList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
