...
/**
 * Table class for FINANCIAL_PERIOD
 */
class FinancialPeriod extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'FINANCIAL_PERIOD';
        $this->TableName = 'FINANCIAL_PERIOD';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[FINANCIAL_PERIOD]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // YEAR_ID
        $this->YEAR_ID = new DbField('FINANCIAL_PERIOD', 'FINANCIAL_PERIOD', 'x_YEAR_ID', 'YEAR_ID', '[YEAR_ID]', 'CAST([YEAR_ID] AS NVARCHAR)', 2, 2, -1, false, '[YEAR_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MONTH_ID
        $this->MONTH_ID = new DbField('FINANCIAL_PERIOD', 'FINANCIAL_PERIOD', 'x_MONTH_ID', 'MONTH_ID', '[MONTH_ID]', 'CAST([MONTH_ID] AS NVARCHAR)', 17, 1, -1, false, '[MONTH_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // START_DATE
        $this->START_DATE = new DbField('FINANCIAL_PERIOD', 'FINANCIAL_PERIOD', 'x_START_DATE', 'START_DATE', '[START_DATE]', CastDateFieldForLike("[START_DATE]", 0, "DB"), 135, 8, 0, false, '[START_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // END_DATE
        $this->END_DATE = new DbField('FINANCIAL_PERIOD', 'FINANCIAL_PERIOD', 'x_END_DATE', 'END_DATE', '[END_DATE]', CastDateFieldForLike("[END_DATE]", 0, "DB"), 135, 8, 0, false, '[END_DATE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[FINANCIAL_PERIOD]";
    }
    ...
    {
        return "[YEAR_ID] = @YEAR_ID@ AND [MONTH_ID] = @MONTH_ID@";
    }
    ...
    public function getKey($current = false)
    {
        ...
        $val = $current ? $this->MONTH_ID->CurrentValue : $this->MONTH_ID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->MONTH_ID->CurrentValue = $keys[1];
            } else {
                $this->MONTH_ID->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("FinancialPeriodList");
    }
    ...
    {
        global $Language;
        if ($pageName == "FinancialPeriodView") {
            return $Language->phrase("View");
        } elseif ($pageName == "FinancialPeriodEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "FinancialPeriodAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "FinancialPeriodAdd";
            case Config("API_EDIT_ACTION"):
                return "FinancialPeriodEdit";
            case Config("API_DELETE_ACTION"):
                return "FinancialPeriodDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("FinancialPeriodView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("FinancialPeriodView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "FinancialPeriodAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "FinancialPeriodAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FinancialPeriodEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("FinancialPeriodAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("FinancialPeriodDelete", $this->getUrlParm());
    }
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",MONTH_ID:" . JsonEncode($this->MONTH_ID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->MONTH_ID->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->MONTH_ID->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("MONTH_ID") ?? Route("MONTH_ID")) !== null) {
                $arKey[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->MONTH_ID->CurrentValue = $key[1];
            } else {
                $this->MONTH_ID->OldValue = $key[1];
            }
