...
/**
 * Table class for INFO_EDU
 */
class InfoEdu extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'INFO_EDU';
        $this->TableName = 'INFO_EDU';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[INFO_EDU]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // YEAR_ID
        $this->YEAR_ID = new DbField('INFO_EDU', 'INFO_EDU', 'x_YEAR_ID', 'YEAR_ID', '[YEAR_ID]', 'CAST([YEAR_ID] AS NVARCHAR)', 2, 2, -1, false, '[YEAR_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // KAL_ID
        $this->KAL_ID = new DbField('INFO_EDU', 'INFO_EDU', 'x_KAL_ID', 'KAL_ID', '[KAL_ID]', '[KAL_ID]', 200, 8, -1, false, '[KAL_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // EDUCATION_TYPE_CODE
        $this->EDUCATION_TYPE_CODE = new DbField('INFO_EDU', 'INFO_EDU', 'x_EDUCATION_TYPE_CODE', 'EDUCATION_TYPE_CODE', '[EDUCATION_TYPE_CODE]', '[EDUCATION_TYPE_CODE]', 200, 3, -1, false, '[EDUCATION_TYPE_CODE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // MALE_NB
        $this->MALE_NB = new DbField('INFO_EDU', 'INFO_EDU', 'x_MALE_NB', 'MALE_NB', '[MALE_NB]', 'CAST([MALE_NB] AS NVARCHAR)', 3, 4, -1, false, '[MALE_NB]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // FEMALE_NB
        $this->FEMALE_NB = new DbField('INFO_EDU', 'INFO_EDU', 'x_FEMALE_NB', 'FEMALE_NB', '[FEMALE_NB]', 'CAST([FEMALE_NB] AS NVARCHAR)', 3, 4, -1, false, '[FEMALE_NB]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[INFO_EDU]";
    }
    ...
    {
        return "[YEAR_ID] = @YEAR_ID@ AND [KAL_ID] = '@KAL_ID@' AND [EDUCATION_TYPE_CODE] = '@EDUCATION_TYPE_CODE@'";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 3) {
            if ($current) {
                $this->YEAR_ID->CurrentValue = $keys[0];
            } else {
                $this->YEAR_ID->OldValue = $keys[0];
            }
            if ($current) {
                $this->KAL_ID->CurrentValue = $keys[1];
            } else {
                $this->KAL_ID->OldValue = $keys[1];
            }
            if ($current) {
                $this->EDUCATION_TYPE_CODE->CurrentValue = $keys[2];
            } else {
                $this->EDUCATION_TYPE_CODE->OldValue = $keys[2];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("InfoEduList");
    }
    ...
    {
        global $Language;
        if ($pageName == "InfoEduView") {
            return $Language->phrase("View");
        } elseif ($pageName == "InfoEduEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "InfoEduAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("InfoEduView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("InfoEduView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "InfoEduAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "InfoEduAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("InfoEduEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("InfoEduAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("InfoEduDelete", $this->getUrlParm());
    }
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",EDUCATION_TYPE_CODE:" . JsonEncode($this->EDUCATION_TYPE_CODE->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->YEAR_ID->CurrentValue = $key[0];
            } else {
                $this->YEAR_ID->OldValue = $key[0];
            }
            if ($setCurrent) {
                $this->KAL_ID->CurrentValue = $key[1];
            } else {
                $this->KAL_ID->OldValue = $key[1];
            }
            if ($setCurrent) {
                $this->EDUCATION_TYPE_CODE->CurrentValue = $key[2];
            } else {
                $this->EDUCATION_TYPE_CODE->OldValue = $key[2];
            }
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->EDUCATION_TYPE_CODE->Raw) {
            $this->EDUCATION_TYPE_CODE->CurrentValue = HtmlDecode($this->EDUCATION_TYPE_CODE->CurrentValue);
        }
