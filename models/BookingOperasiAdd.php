 ...
 * Page class
 */
class BookingOperasiAdd extends BookingOperasi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'booking_operasi';

    // Page object name
    public $PageObjName = "BookingOperasiAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (booking_operasi)
        if (!isset($GLOBALS["booking_operasi"]) || get_class($GLOBALS["booking_operasi"]) == PROJECT_NAMESPACE . "booking_operasi") {
            $GLOBALS["booking_operasi"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'booking_operasi');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("booking_operasi"));
                $doc->Text = @$content;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("BookingOperasiList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "BookingOperasiList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "BookingOperasiView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->kode_paket->CurrentValue = null;
        $this->kode_paket->OldValue = $this->kode_paket->CurrentValue;
        ...
        $this->kd_ruang_ok->CurrentValue = null;
        $this->kd_ruang_ok->OldValue = $this->kd_ruang_ok->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'kode_paket' first before field var 'x_kode_paket'
        $val = $CurrentForm->hasValue("kode_paket") ? $CurrentForm->getValue("kode_paket") : $CurrentForm->getValue("x_kode_paket");
        if (!$this->kode_paket->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->kode_paket->Visible = false; // Disable update for API request
            } else {
                $this->kode_paket->setFormValue($val);
            }
        }
        ...
        // Check field name 'kd_ruang_ok' first before field var 'x_kd_ruang_ok'
        $val = $CurrentForm->hasValue("kd_ruang_ok") ? $CurrentForm->getValue("kd_ruang_ok") : $CurrentForm->getValue("x_kd_ruang_ok");
        if (!$this->kd_ruang_ok->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->kd_ruang_ok->Visible = false; // Disable update for API request
            } else {
                $this->kd_ruang_ok->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->kode_paket->CurrentValue = $this->kode_paket->FormValue;
        ...
        $this->kd_ruang_ok->CurrentValue = $this->kd_ruang_ok->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['kode_paket'] = $this->kode_paket->CurrentValue;
        $row['tanggal'] = $this->tanggal->CurrentValue;
        $row['jam_mulai'] = $this->jam_mulai->CurrentValue;
        $row['jam_selesai'] = $this->jam_selesai->CurrentValue;
        $row['status'] = $this->status->CurrentValue;
        $row['kd_dokter'] = $this->kd_dokter->CurrentValue;
        $row['kd_ruang_ok'] = $this->kd_ruang_ok->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->kode_paket->EditValue = HtmlEncode($this->kode_paket->CurrentValue);
            ...
            $this->jam_mulai->EditValue = HtmlEncode(FormatDateTime($this->jam_mulai->CurrentValue, 4));
            ...
            $this->jam_selesai->EditValue = HtmlEncode(FormatDateTime($this->jam_selesai->CurrentValue, 4));
            ...
            $this->kd_ruang_ok->EditValue = HtmlEncode($this->kd_ruang_ok->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->kode_paket->Required) {
            if (!$this->kode_paket->IsDetailKey && EmptyValue($this->kode_paket->FormValue)) {
                $this->kode_paket->addErrorMessage(str_replace("%s", $this->kode_paket->caption(), $this->kode_paket->RequiredErrorMessage));
            }
        }
        ...
        if ($this->jam_mulai->Required) {
            if (!$this->jam_mulai->IsDetailKey && EmptyValue($this->jam_mulai->FormValue)) {
        ...
        if ($this->jam_selesai->Required) {
            if (!$this->jam_selesai->IsDetailKey && EmptyValue($this->jam_selesai->FormValue)) {
        ...
        if ($this->kd_ruang_ok->Required) {
            if (!$this->kd_ruang_ok->IsDetailKey && EmptyValue($this->kd_ruang_ok->FormValue)) {
                $this->kd_ruang_ok->addErrorMessage(str_replace("%s", $this->kd_ruang_ok->caption(), $this->kd_ruang_ok->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // kode_paket
        $this->kode_paket->setDbValueDef($rsnew, $this->kode_paket->CurrentValue, null, false);

        // tanggal
        $this->tanggal->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal->CurrentValue, 0), null, false);

        // jam_mulai
        $this->jam_mulai->setDbValueDef($rsnew, UnFormatDateTime($this->jam_mulai->CurrentValue, 4), null, false);

        // jam_selesai
        $this->jam_selesai->setDbValueDef($rsnew, UnFormatDateTime($this->jam_selesai->CurrentValue, 4), null, false);
        ...
        // kd_ruang_ok
        $this->kd_ruang_ok->setDbValueDef($rsnew, $this->kd_ruang_ok->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("BookingOperasiList"), "", $this->TableVar, true);
