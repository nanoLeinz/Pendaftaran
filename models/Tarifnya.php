...
/**
 * Table class for tarifnya
 */
class Tarifnya extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'tarifnya';
        $this->TableName = 'tarifnya';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[tarifnya]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // tarif_id
        $this->tarif_id = new DbField('tarifnya', 'tarifnya', 'x_tarif_id', 'tarif_id', '[tarif_id]', '[tarif_id]', 200, 25, -1, false, '[tarif_id]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // harga
        $this->harga = new DbField('tarifnya', 'tarifnya', 'x_harga', 'harga', '[harga]', 'CAST([harga] AS NVARCHAR)', 6, 8, -1, false, '[harga]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[tarifnya]";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TarifnyaList");
    }
    ...
    {
        global $Language;
        if ($pageName == "TarifnyaView") {
            return $Language->phrase("View");
        } elseif ($pageName == "TarifnyaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "TarifnyaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TarifnyaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("TarifnyaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "TarifnyaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "TarifnyaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("TarifnyaEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifnyaAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("TarifnyaDelete", $this->getUrlParm());
    }
