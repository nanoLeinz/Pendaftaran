...
class VLaboratoriumList extends VLaboratorium
{
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "VLaboratoriumAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'TREATMENT_BILL')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("TreatmentBillList?" . Config("TABLE_SHOW_MASTER") . "=V_LABORATORIUM&" . GetForeignKeyUrl("fk_VISIT_ID", $this->VISIT_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("TreatmentBillGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'V_LABORATORIUM')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'V_LABORATORIUM')) {
