...
/**
 * Table class for nobpjs
 */
class Nobpjs extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'nobpjs';
        $this->TableName = 'nobpjs';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[nobpjs]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // kk_no
        $this->kk_no = new DbField('nobpjs', 'nobpjs', 'x_kk_no', 'kk_no', '[kk_no]', '[kk_no]', 200, 30, -1, false, '[kk_no]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // jml
        $this->jml = new DbField('nobpjs', 'nobpjs', 'x_jml', 'jml', '[jml]', 'CAST([jml] AS NVARCHAR)', 3, 4, -1, false, '[jml]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->jml->Sortable = true; // Allow sort
        $this->jml->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[nobpjs]";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("NobpjsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "NobpjsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "NobpjsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "NobpjsAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("NobpjsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("NobpjsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "NobpjsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "NobpjsAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("NobpjsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("NobpjsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("NobpjsDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        $this->jml->ViewValue = FormatNumber($this->jml->ViewValue, 0, -2, -2, -2);
