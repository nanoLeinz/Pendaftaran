 ...
 * Page class
 */
class FamilyList extends Family
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FamilyList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fFAMILYlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "FamilyAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "FamilyDelete";
        $this->MultiUpdateUrl = "FamilyUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fFAMILYlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->FAMILY_ID->AdvancedSearch->toJson(), ","); // Field FAMILY_ID
        $filterList = Concat($filterList, $this->FAMILY_STATUS_ID->AdvancedSearch->toJson(), ","); // Field FAMILY_STATUS_ID
        $filterList = Concat($filterList, $this->NO_REGISTRATION2->AdvancedSearch->toJson(), ","); // Field NO_REGISTRATION2
        $filterList = Concat($filterList, $this->FULLNAME->AdvancedSearch->toJson(), ","); // Field FULLNAME
        $filterList = Concat($filterList, $this->ISRESPONSIBLE->AdvancedSearch->toJson(), ","); // Field ISRESPONSIBLE
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fFAMILYlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field FAMILY_ID
        $this->FAMILY_ID->AdvancedSearch->SearchValue = @$filter["x_FAMILY_ID"];
        $this->FAMILY_ID->AdvancedSearch->SearchOperator = @$filter["z_FAMILY_ID"];
        $this->FAMILY_ID->AdvancedSearch->SearchCondition = @$filter["v_FAMILY_ID"];
        $this->FAMILY_ID->AdvancedSearch->SearchValue2 = @$filter["y_FAMILY_ID"];
        $this->FAMILY_ID->AdvancedSearch->SearchOperator2 = @$filter["w_FAMILY_ID"];
        $this->FAMILY_ID->AdvancedSearch->save();
        ...
        // Field NO_REGISTRATION2
        $this->NO_REGISTRATION2->AdvancedSearch->SearchValue = @$filter["x_NO_REGISTRATION2"];
        $this->NO_REGISTRATION2->AdvancedSearch->SearchOperator = @$filter["z_NO_REGISTRATION2"];
        $this->NO_REGISTRATION2->AdvancedSearch->SearchCondition = @$filter["v_NO_REGISTRATION2"];
        $this->NO_REGISTRATION2->AdvancedSearch->SearchValue2 = @$filter["y_NO_REGISTRATION2"];
        $this->NO_REGISTRATION2->AdvancedSearch->SearchOperator2 = @$filter["w_NO_REGISTRATION2"];
        $this->NO_REGISTRATION2->AdvancedSearch->save();
        ...
        // Field ISRESPONSIBLE
        $this->ISRESPONSIBLE->AdvancedSearch->SearchValue = @$filter["x_ISRESPONSIBLE"];
        $this->ISRESPONSIBLE->AdvancedSearch->SearchOperator = @$filter["z_ISRESPONSIBLE"];
        $this->ISRESPONSIBLE->AdvancedSearch->SearchCondition = @$filter["v_ISRESPONSIBLE"];
        $this->ISRESPONSIBLE->AdvancedSearch->SearchValue2 = @$filter["y_ISRESPONSIBLE"];
        $this->ISRESPONSIBLE->AdvancedSearch->SearchOperator2 = @$filter["w_ISRESPONSIBLE"];
        $this->ISRESPONSIBLE->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->NO_REGISTRATION2, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->FULLNAME, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->ISRESPONSIBLE, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->FAMILY_ID); // FAMILY_ID
            $this->updateSort($this->FAMILY_STATUS_ID); // FAMILY_STATUS_ID
            $this->updateSort($this->NO_REGISTRATION2); // NO_REGISTRATION2
            $this->updateSort($this->FULLNAME); // FULLNAME
            $this->updateSort($this->ISRESPONSIBLE); // ISRESPONSIBLE
            ...
            $this->updateSort($this->EDUCATION_TYPE_CODE); // EDUCATION_TYPE_CODE
            $this->updateSort($this->JOB_ID); // JOB_ID
            $this->updateSort($this->BLOOD_ID); // BLOOD_ID
            $this->updateSort($this->MARITALSTATUSID); // MARITALSTATUSID
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->FAMILY_ID->setSort("");
                $this->FAMILY_STATUS_ID->setSort("");
                $this->NO_REGISTRATION2->setSort("");
                $this->FULLNAME->setSort("");
                $this->ISRESPONSIBLE->setSort("");
                $this->GENDER->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->ORG_UNIT_CODE->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->NO_REGISTRATION->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->FAMILY_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fFAMILYlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fFAMILYlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fFAMILYlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fFAMILYlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
