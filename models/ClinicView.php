 ...
 * Page class
 */
class ClinicView extends Clinic
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("CLINIC_ID") ?? Route("CLINIC_ID")) !== null) {
            $this->RecKey["CLINIC_ID"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("ORG_UNIT_CODE") ?? Route("ORG_UNIT_CODE")) !== null) {
                ...
                $returnUrl = "ClinicList"; // Return to list
            }
            if (($keyValue = Get("CLINIC_ID") ?? Route("CLINIC_ID")) !== null) {
                $this->CLINIC_ID->setQueryStringValue($keyValue);
                $this->RecKey["CLINIC_ID"] = $this->CLINIC_ID->QueryStringValue;
            } elseif (Post("CLINIC_ID") !== null) {
                $this->CLINIC_ID->setFormValue(Post("CLINIC_ID"));
                $this->RecKey["CLINIC_ID"] = $this->CLINIC_ID->FormValue;
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ClinicList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "ClinicList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
