...
/**
 * Table class for SIGNEDBY
 */
class Signedby extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'SIGNEDBY';
        $this->TableName = 'SIGNEDBY';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[SIGNEDBY]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // SIGN_ID
        $this->SIGN_ID = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_SIGN_ID', 'SIGN_ID', '[SIGN_ID]', 'CAST([SIGN_ID] AS NVARCHAR)', 3, 4, -1, false, '[SIGN_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // TITLE
        $this->TITLE = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_TITLE', 'TITLE', '[TITLE]', '[TITLE]', 200, 150, -1, false, '[TITLE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // FULLNAME
        $this->FULLNAME = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_FULLNAME', 'FULLNAME', '[FULLNAME]', '[FULLNAME]', 200, 200, -1, false, '[FULLNAME]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // GELAR
        $this->GELAR = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_GELAR', 'GELAR', '[GELAR]', '[GELAR]', 200, 150, -1, false, '[GELAR]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // EMPLOYEE_ID
        $this->EMPLOYEE_ID = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_EMPLOYEE_ID', 'EMPLOYEE_ID', '[EMPLOYEE_ID]', '[EMPLOYEE_ID]', 200, 50, -1, false, '[EMPLOYEE_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // DESCRIPTION
        $this->DESCRIPTION = new DbField('SIGNEDBY', 'SIGNEDBY', 'x_DESCRIPTION', 'DESCRIPTION', '[DESCRIPTION]', '[DESCRIPTION]', 200, 200, -1, false, '[DESCRIPTION]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[SIGNEDBY]";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->SIGN_ID->CurrentValue = $keys[0];
            } else {
                $this->SIGN_ID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SignedbyList");
    }
    ...
    {
        global $Language;
        if ($pageName == "SignedbyView") {
            return $Language->phrase("View");
        } elseif ($pageName == "SignedbyEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SignedbyAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SignedbyView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("SignedbyView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "SignedbyAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "SignedbyAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("SignedbyEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SignedbyAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("SignedbyDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->SIGN_ID->CurrentValue = $key;
            } else {
                $this->SIGN_ID->OldValue = $key;
            }
