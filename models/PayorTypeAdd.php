 ...
 * Page class
 */
class PayorTypeAdd extends PayorType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PayorTypeAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("PAYOR_TYPE") ?? Route("PAYOR_TYPE")) !== null) {
                $this->PAYOR_TYPE->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PayorTypeList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PayorTypeList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PayorTypeView") {
    ...
    {
        $this->PAYOR_TYPE->CurrentValue = null;
        $this->PAYOR_TYPE->OldValue = $this->PAYOR_TYPE->CurrentValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['PAYOR_TYPE'] = $this->PAYOR_TYPE->CurrentValue;
    ...
    {
        global $Language, $Security;
        if ($this->PAYOR_TYPE->CurrentValue != "") { // Check field with unique index
            $filter = "([PAYOR_TYPE] = " . AdjustSql($this->PAYOR_TYPE->CurrentValue, $this->Dbid) . ")";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->PAYOR_TYPE->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->PAYOR_TYPE->CurrentValue, $idxErrMsg);
        ...
        // PAYOR_TYPE
        $this->PAYOR_TYPE->setDbValueDef($rsnew, $this->PAYOR_TYPE->CurrentValue, 0, false);
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['PAYOR_TYPE']) == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PayorTypeList"), "", $this->TableVar, true);
