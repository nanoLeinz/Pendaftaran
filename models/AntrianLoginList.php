 ...
 * Page class
 */
class AntrianLoginList extends AntrianLogin
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AntrianLoginList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fANTRIAN_LOGINlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "AntrianLoginAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "AntrianLoginDelete";
        $this->MultiUpdateUrl = "AntrianLoginUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fANTRIAN_LOGINlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->TANGGAL_REGIS->AdvancedSearch->toJson(), ","); // Field TANGGAL_REGIS
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fANTRIAN_LOGINlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field TANGGAL_REGIS
        $this->TANGGAL_REGIS->AdvancedSearch->SearchValue = @$filter["x_TANGGAL_REGIS"];
        $this->TANGGAL_REGIS->AdvancedSearch->SearchOperator = @$filter["z_TANGGAL_REGIS"];
        $this->TANGGAL_REGIS->AdvancedSearch->SearchCondition = @$filter["v_TANGGAL_REGIS"];
        $this->TANGGAL_REGIS->AdvancedSearch->SearchValue2 = @$filter["y_TANGGAL_REGIS"];
        $this->TANGGAL_REGIS->AdvancedSearch->SearchOperator2 = @$filter["w_TANGGAL_REGIS"];
        $this->TANGGAL_REGIS->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->NOMR, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->NO_BPJS, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->TANGGAL_REGIS); // TANGGAL_REGIS
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fANTRIAN_LOGINlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fANTRIAN_LOGINlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fANTRIAN_LOGINlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fANTRIAN_LOGINlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
