 ...
 * Page class
 */
class PayorInfoAdd extends PayorInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PayorInfoAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "PayorInfoView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->KDVKLAIM->CurrentValue = null;
        $this->KDVKLAIM->OldValue = $this->KDVKLAIM->CurrentValue;
    }
    ...
    {
        global $Language, $Security;
        if ($this->PAYOR_ID->CurrentValue != "") { // Check field with unique index
            $filter = "([PAYOR_ID] = '" . AdjustSql($this->PAYOR_ID->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->PAYOR_ID->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->PAYOR_ID->CurrentValue, $idxErrMsg);
        ...
        // PAYOR_ID
        $this->PAYOR_ID->setDbValueDef($rsnew, $this->PAYOR_ID->CurrentValue, "", strval($this->PAYOR_ID->CurrentValue) == "");

        // PAYOR_TYPE
        $this->PAYOR_TYPE->setDbValueDef($rsnew, $this->PAYOR_TYPE->CurrentValue, null, false);

        // PAYOR
        $this->PAYOR->setDbValueDef($rsnew, $this->PAYOR->CurrentValue, null, false);
        ...
        // KDVKLAIM
        $this->KDVKLAIM->setDbValueDef($rsnew, $this->KDVKLAIM->CurrentValue, null, false);

        // Call Row Inserting event
        $insertRow = $this->rowInserting($rsold, $rsnew);

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['PAYOR_ID']) == "") {
