 ...
 * Page class
 */
class RegisterCaraBayarSummary extends RegisterCaraBayar
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'register_cara_bayar';

    // Page object name
    public $PageObjName = "RegisterCaraBayarSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (register_cara_bayar)
        if (!isset($GLOBALS["register_cara_bayar"]) || get_class($GLOBALS["register_cara_bayar"]) == PROJECT_NAMESPACE . "register_cara_bayar") {
            $GLOBALS["register_cara_bayar"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'register_cara_bayar');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            ...
            $curVal = $this->STATUS_PASIEN_ID->AdvancedSearch->SearchValue == INIT_VALUE ? "" : trim(strval($this->STATUS_PASIEN_ID->AdvancedSearch->SearchValue));
            if ($curVal != "") {
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd", "") == "reset") {
            ...
            $this->getDropDownValue($this->STATUS_PASIEN_ID);
        ...
        if ($restoreSession) {
            ...
            if ($this->STATUS_PASIEN_ID->AdvancedSearch->issetSession()) { // Field STATUS_PASIEN_ID
        ...
        $this->buildDropDownFilter($this->STATUS_PASIEN_ID, $filter, $this->STATUS_PASIEN_ID->AdvancedSearch->SearchOperator, false, true); // Field STATUS_PASIEN_ID
        $this->STATUS_PASIEN_ID->AdvancedSearch->save();

        // Field STATUS_PASIEN_ID
        LoadDropDownList($this->STATUS_PASIEN_ID->EditValue, $this->STATUS_PASIEN_ID->AdvancedSearch->SearchValue);
        return $filter;
    }
    ...
    protected function loadDefaultFilters()
    {
        ...
        // Field STATUS_PASIEN_ID
        $this->STATUS_PASIEN_ID->AdvancedSearch->loadDefault();
    }
    ...
    public function showFilterList()
    {
        ...
        // Field STATUS_PASIEN_ID
        $extWrk = "";
        $this->buildDropDownFilter($this->STATUS_PASIEN_ID, $extWrk, $this->STATUS_PASIEN_ID->AdvancedSearch->SearchOperator);
        $filter = "";
        if ($extWrk != "") {
            $filter .= "<span class=\"ew-filter-value\">$extWrk</span>";
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->STATUS_PASIEN_ID->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function getFilterList()
    {
        ...
        // Field STATUS_PASIEN_ID
        $wrk = "";
        $wrk = ($this->STATUS_PASIEN_ID->AdvancedSearch->SearchValue != INIT_VALUE) ? $this->STATUS_PASIEN_ID->AdvancedSearch->SearchValue : "";
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
            $wrk = "\"x_STATUS_PASIEN_ID\":\"" . JsEncode($wrk) . "\"";
        }
        if ($wrk != "") {
            if ($filterList != "") {
                $filterList .= ",";
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field STATUS_PASIEN_ID
        if (!$this->STATUS_PASIEN_ID->AdvancedSearch->getFromArray($filter)) {
            $this->STATUS_PASIEN_ID->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->STATUS_PASIEN_ID->AdvancedSearch->save();
        return true;
    }
