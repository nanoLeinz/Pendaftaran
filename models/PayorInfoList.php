 ...
 * Page class
 */
class PayorInfoList extends PayorInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PayorInfoList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fPAYOR_INFOlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PayorInfoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PayorInfoDelete";
        $this->MultiUpdateUrl = "PayorInfoUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fPAYOR_INFOlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->PAYOR_TYPE->AdvancedSearch->toJson(), ","); // Field PAYOR_TYPE
        $filterList = Concat($filterList, $this->PAYOR->AdvancedSearch->toJson(), ","); // Field PAYOR
        ...
        $filterList = Concat($filterList, $this->KDVKLAIM->AdvancedSearch->toJson(), ","); // Field KDVKLAIM
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fPAYOR_INFOlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field PAYOR_TYPE
        $this->PAYOR_TYPE->AdvancedSearch->SearchValue = @$filter["x_PAYOR_TYPE"];
        $this->PAYOR_TYPE->AdvancedSearch->SearchOperator = @$filter["z_PAYOR_TYPE"];
        $this->PAYOR_TYPE->AdvancedSearch->SearchCondition = @$filter["v_PAYOR_TYPE"];
        $this->PAYOR_TYPE->AdvancedSearch->SearchValue2 = @$filter["y_PAYOR_TYPE"];
        $this->PAYOR_TYPE->AdvancedSearch->SearchOperator2 = @$filter["w_PAYOR_TYPE"];
        $this->PAYOR_TYPE->AdvancedSearch->save();

        // Field PAYOR
        $this->PAYOR->AdvancedSearch->SearchValue = @$filter["x_PAYOR"];
        $this->PAYOR->AdvancedSearch->SearchOperator = @$filter["z_PAYOR"];
        $this->PAYOR->AdvancedSearch->SearchCondition = @$filter["v_PAYOR"];
        $this->PAYOR->AdvancedSearch->SearchValue2 = @$filter["y_PAYOR"];
        $this->PAYOR->AdvancedSearch->SearchOperator2 = @$filter["w_PAYOR"];
        $this->PAYOR->AdvancedSearch->save();
        ...
        // Field KDVKLAIM
        $this->KDVKLAIM->AdvancedSearch->SearchValue = @$filter["x_KDVKLAIM"];
        $this->KDVKLAIM->AdvancedSearch->SearchOperator = @$filter["z_KDVKLAIM"];
        $this->KDVKLAIM->AdvancedSearch->SearchCondition = @$filter["v_KDVKLAIM"];
        $this->KDVKLAIM->AdvancedSearch->SearchValue2 = @$filter["y_KDVKLAIM"];
        $this->KDVKLAIM->AdvancedSearch->SearchOperator2 = @$filter["w_KDVKLAIM"];
        $this->KDVKLAIM->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->PAYOR, $arKeywords, $type);
        ...
        $this->buildBasicSearchSql($where, $this->KDVKLAIM, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->PAYOR_TYPE); // PAYOR_TYPE
            $this->updateSort($this->PAYOR); // PAYOR
            $this->updateSort($this->ADDRESS); // ADDRESS
            $this->updateSort($this->CITY); // CITY
            $this->updateSort($this->PHONE); // PHONE
            $this->updateSort($this->FAX); // FAX
            $this->updateSort($this->KDVKLAIM); // KDVKLAIM
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->PAYOR_TYPE->setSort("");
                $this->PAYOR->setSort("");
                $this->ADDRESS->setSort("");
                $this->CITY->setSort("");
                $this->PHONE->setSort("");
                $this->FAX->setSort("");
                $this->KDVKLAIM->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->PAYOR_ID->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fPAYOR_INFOlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fPAYOR_INFOlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fPAYOR_INFOlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fPAYOR_INFOlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
