...
/**
 * Table class for SERVICE_TYPE
 */
class ServiceType extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'SERVICE_TYPE';
        $this->TableName = 'SERVICE_TYPE';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[SERVICE_TYPE]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // STYPE_ID
        $this->STYPE_ID = new DbField('SERVICE_TYPE', 'SERVICE_TYPE', 'x_STYPE_ID', 'STYPE_ID', '[STYPE_ID]', 'CAST([STYPE_ID] AS NVARCHAR)', 2, 2, -1, false, '[STYPE_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // SERVICE_TYPE
        $this->SERVICE_TYPE = new DbField('SERVICE_TYPE', 'SERVICE_TYPE', 'x_SERVICE_TYPE', 'SERVICE_TYPE', '[SERVICE_TYPE]', '[SERVICE_TYPE]', 200, 50, -1, false, '[SERVICE_TYPE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[SERVICE_TYPE]";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->STYPE_ID->CurrentValue = $keys[0];
            } else {
                $this->STYPE_ID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ServiceTypeList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ServiceTypeView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ServiceTypeEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ServiceTypeAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ServiceTypeView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ServiceTypeView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ServiceTypeAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ServiceTypeAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("ServiceTypeEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServiceTypeAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ServiceTypeDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->STYPE_ID->CurrentValue = $key;
            } else {
                $this->STYPE_ID->OldValue = $key;
            }
