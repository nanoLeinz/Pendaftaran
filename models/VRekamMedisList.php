 ...
 * Page class
 */
class VRekamMedisList extends VRekamMedis
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VRekamMedisList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fV_REKAM_MEDISlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "VRekamMedisAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "VRekamMedisDelete";
        $this->MultiUpdateUrl = "VRekamMedisUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fV_REKAM_MEDISlistsrch";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->CETAK_DOC->AdvancedSearch->toJson(), ","); // Field CETAK_DOC
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fV_REKAM_MEDISlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field CETAK_DOC
        $this->CETAK_DOC->AdvancedSearch->SearchValue = @$filter["x_CETAK_DOC"];
        $this->CETAK_DOC->AdvancedSearch->SearchOperator = @$filter["z_CETAK_DOC"];
        $this->CETAK_DOC->AdvancedSearch->SearchCondition = @$filter["v_CETAK_DOC"];
        $this->CETAK_DOC->AdvancedSearch->SearchValue2 = @$filter["y_CETAK_DOC"];
        $this->CETAK_DOC->AdvancedSearch->SearchOperator2 = @$filter["w_CETAK_DOC"];
        $this->CETAK_DOC->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->CETAK_DOC, $default, false); // CETAK_DOC
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->CETAK_DOC->AdvancedSearch->save(); // CETAK_DOC
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->CETAK_DOC->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->CETAK_DOC->AdvancedSearch->unsetSession();
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fV_REKAM_MEDISlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fV_REKAM_MEDISlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fV_REKAM_MEDISlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // CETAK_DOC
        if (!$this->isAddOrEdit() && $this->CETAK_DOC->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->CETAK_DOC->AdvancedSearch->SearchValue != "" || $this->CETAK_DOC->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fV_REKAM_MEDISlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"VRekamMedisSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        $item->Visible = true;
