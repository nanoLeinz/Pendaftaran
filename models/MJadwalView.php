 ...
 * Page class
 */
class MJadwalView extends MJadwal
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MJadwalView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("id_jadwal") ?? Route("id_jadwal")) !== null) {
            $this->RecKey["id_jadwal"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id_jadwal") ?? Route("id_jadwal")) !== null) {
                ...
                $this->RecKey["id_jadwal"] = $this->id_jadwal->QueryStringValue;
            } elseif (Post("id_jadwal") !== null) {
                $this->id_jadwal->setFormValue(Post("id_jadwal"));
                $this->RecKey["id_jadwal"] = $this->id_jadwal->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->id_jadwal->setQueryStringValue($keyValue);
                $this->RecKey["id_jadwal"] = $this->id_jadwal->QueryStringValue;
            } else {
                $returnUrl = "MJadwalList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "MJadwalList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "MJadwalList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
