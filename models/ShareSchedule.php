...
/**
 * Table class for SHARE_SCHEDULE
 */
class ShareSchedule extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'SHARE_SCHEDULE';
        $this->TableName = 'SHARE_SCHEDULE';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "[dbo].[SHARE_SCHEDULE]";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // ORG_UNIT_CODE
        $this->ORG_UNIT_CODE = new DbField('SHARE_SCHEDULE', 'SHARE_SCHEDULE', 'x_ORG_UNIT_CODE', 'ORG_UNIT_CODE', '[ORG_UNIT_CODE]', '[ORG_UNIT_CODE]', 200, 50, -1, false, '[ORG_UNIT_CODE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // TASK_ID
        $this->TASK_ID = new DbField('SHARE_SCHEDULE', 'SHARE_SCHEDULE', 'x_TASK_ID', 'TASK_ID', '[TASK_ID]', 'CAST([TASK_ID] AS NVARCHAR)', 2, 2, -1, false, '[TASK_ID]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // COMP_TYPE
        $this->COMP_TYPE = new DbField('SHARE_SCHEDULE', 'SHARE_SCHEDULE', 'x_COMP_TYPE', 'COMP_TYPE', '[COMP_TYPE]', 'CAST([COMP_TYPE] AS NVARCHAR)', 17, 1, -1, false, '[COMP_TYPE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->COMP_TYPE->IsPrimaryKey = true; // Primary key field
        $this->COMP_TYPE->Nullable = false; // NOT NULL field
        $this->COMP_TYPE->Required = true; // Required field
        ...
        // PERCENTAGE
        $this->PERCENTAGE = new DbField('SHARE_SCHEDULE', 'SHARE_SCHEDULE', 'x_PERCENTAGE', 'PERCENTAGE', '[PERCENTAGE]', 'CAST([PERCENTAGE] AS NVARCHAR)', 131, 8, -1, false, '[PERCENTAGE]', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // FA_V
        $this->FA_V = new DbField('SHARE_SCHEDULE', 'SHARE_SCHEDULE', 'x_FA_V', 'FA_V', '[FA_V]', 'CAST([FA_V] AS NVARCHAR)', 2, 2, -1, false, '[FA_V]', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[SHARE_SCHEDULE]";
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            ...
            if (array_key_exists('COMP_TYPE', $rs)) {
                AddFilter($where, QuotedName('COMP_TYPE', $this->Dbid) . '=' . QuotedValue($rs['COMP_TYPE'], $this->COMP_TYPE->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "[ORG_UNIT_CODE] = '@ORG_UNIT_CODE@' AND [TASK_ID] = @TASK_ID@ AND [COMP_TYPE] = @COMP_TYPE@";
    }
    ...
    public function getKey($current = false)
    {
        ...
        $val = $current ? $this->COMP_TYPE->CurrentValue : $this->COMP_TYPE->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 3) {
            ...
            if ($current) {
                $this->TASK_ID->CurrentValue = $keys[1];
            } else {
                $this->TASK_ID->OldValue = $keys[1];
            }
            if ($current) {
                $this->COMP_TYPE->CurrentValue = $keys[2];
            } else {
                $this->COMP_TYPE->OldValue = $keys[2];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('COMP_TYPE', $row) ? $row['COMP_TYPE'] : null;
        } else {
            $val = $this->COMP_TYPE->OldValue !== null ? $this->COMP_TYPE->OldValue : $this->COMP_TYPE->CurrentValue;
        }
        if (!is_numeric($val)) {
            return "0=1"; // Invalid key
        }
        if ($val === null) {
            ...
            $keyFilter = str_replace("@COMP_TYPE@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ShareScheduleList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ShareScheduleView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ShareScheduleEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ShareScheduleAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "ShareScheduleView";
            case Config("API_ADD_ACTION"):
                return "ShareScheduleAdd";
            case Config("API_EDIT_ACTION"):
                return "ShareScheduleEdit";
            case Config("API_DELETE_ACTION"):
                return "ShareScheduleDelete";
            case Config("API_LIST_ACTION"):
                return "ShareScheduleList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ShareScheduleView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ShareScheduleView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ShareScheduleAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ShareScheduleAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ShareScheduleEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ShareScheduleAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ShareScheduleDelete", $this->getUrlParm());
    }
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",COMP_TYPE:" . JsonEncode($this->COMP_TYPE->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->COMP_TYPE->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->COMP_TYPE->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("COMP_TYPE") ?? Route("COMP_TYPE")) !== null) {
                $arKey[] = $keyValue;
        ...
        if (is_array($arKeys)) {
            foreach ($arKeys as $key) {
                ...
                if (!is_numeric($key[1])) { // TASK_ID
                    continue;
                }
                if (!is_numeric($key[2])) { // COMP_TYPE
                    continue;
                }
                $ar[] = $key;
            }
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->TASK_ID->CurrentValue = $key[1];
            } else {
                $this->TASK_ID->OldValue = $key[1];
            }
            if ($setCurrent) {
                $this->COMP_TYPE->CurrentValue = $key[2];
            } else {
                $this->COMP_TYPE->OldValue = $key[2];
            }
