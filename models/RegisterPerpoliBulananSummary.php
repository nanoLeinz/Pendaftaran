 ...
 * Page class
 */
class RegisterPerpoliBulananSummary extends RegisterPerpoliBulanan
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'register_perpoli_bulanan';

    // Page object name
    public $PageObjName = "RegisterPerpoliBulananSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (register_perpoli_bulanan)
        if (!isset($GLOBALS["register_perpoli_bulanan"]) || get_class($GLOBALS["register_perpoli_bulanan"]) == PROJECT_NAMESPACE . "register_perpoli_bulanan") {
            $GLOBALS["register_perpoli_bulanan"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'register_perpoli_bulanan');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            ...
            $curVal = $this->MONTH->AdvancedSearch->SearchValue == INIT_VALUE ? "" : trim(strval($this->MONTH->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->MONTH->AdvancedSearch->ViewValue = $this->MONTH->lookupCacheOption($curVal);
            } else {
                $this->MONTH->AdvancedSearch->ViewValue = $this->MONTH->Lookup !== null && is_array($this->MONTH->Lookup->Options) ? $curVal : null;
            }
            if ($this->MONTH->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->MONTH->EditValue = array_values($this->MONTH->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[MONTH_ID]" . SearchString("=", $this->MONTH->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->MONTH->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            $curVal = $this->YEAR->AdvancedSearch->SearchValue == INIT_VALUE ? "" : trim(strval($this->YEAR->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->YEAR->AdvancedSearch->ViewValue = $this->YEAR->lookupCacheOption($curVal);
            } else {
                $this->YEAR->AdvancedSearch->ViewValue = $this->YEAR->Lookup !== null && is_array($this->YEAR->Lookup->Options) ? $curVal : null;
            }
            if ($this->YEAR->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->YEAR->EditValue = array_values($this->YEAR->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "[YEAR_ID]" . SearchString("=", $this->YEAR->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->YEAR->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                foreach ($arwrk as &$row)
                    $row = $this->YEAR->Lookup->renderViewRow($row);
            ...
            $curVal = $this->PAYOR_ID->CurrentValue == INIT_VALUE ? "" : trim(strval($this->PAYOR_ID->CurrentValue));
            if ($curVal != "") {
            ...
            $curVal = $this->MONTH->CurrentValue == INIT_VALUE ? "" : trim(strval($this->MONTH->CurrentValue));
            if ($curVal != "") {
            ...
            $curVal = $this->YEAR->CurrentValue == INIT_VALUE ? "" : trim(strval($this->YEAR->CurrentValue));
            if ($curVal != "") {
        ...
        if ($this->RowType == ROWTYPE_TOTAL) { // Summary row
            ...
            $this->cellRendered($this->MONTH, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<a class="ew-export-link ew-email" title="' . HtmlEncode($Language->phrase("ExportToEmail", true)) . '" data-caption="' . HtmlEncode($Language->phrase("ExportToEmail", true)) . '" id="emf_register_perpoli_bulanan" href="#" onclick="return ew.emailDialogShow({ lnk: \'emf_register_perpoli_bulanan\', hdr: ew.language.phrase(\'ExportToEmailText\'), url: \'' . $url . '\', exportid: \'' . session_id() . '\', el: this });">' . $Language->phrase("ExportToEmail") . '</a>';
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd", "") == "reset") {
            ...
            // Field MONTH
            $this->getDropDownValue($this->MONTH);

            // Field YEAR
            $this->getDropDownValue($this->YEAR);
        ...
        if ($restoreSession) {
            ...
            if ($this->MONTH->AdvancedSearch->issetSession()) { // Field MONTH
                $this->MONTH->AdvancedSearch->load();
                $restoreDefault = false;
            }
            if ($this->YEAR->AdvancedSearch->issetSession()) { // Field YEAR
        ...
        // Field MONTH
        LoadDropDownList($this->MONTH->EditValue, $this->MONTH->AdvancedSearch->SearchValue);

        // Field YEAR
        LoadDropDownList($this->YEAR->EditValue, $this->YEAR->AdvancedSearch->SearchValue);
        return $filter;
    }
    ...
    protected function loadDefaultFilters()
    {
        ...
        // Field MONTH
        $this->MONTH->AdvancedSearch->loadDefault();
    ...
    public function getFilterList()
    {
        ...
        // Field MONTH
        $wrk = "";
        $wrk = ($this->MONTH->AdvancedSearch->SearchValue != INIT_VALUE) ? $this->MONTH->AdvancedSearch->SearchValue : "";
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
        ...
        // Field YEAR
        $wrk = "";
        $wrk = ($this->YEAR->AdvancedSearch->SearchValue != INIT_VALUE) ? $this->YEAR->AdvancedSearch->SearchValue : "";
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field MONTH
        if (!$this->MONTH->AdvancedSearch->getFromArray($filter)) {
            $this->MONTH->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->MONTH->AdvancedSearch->save();

        // Field YEAR
        if (!$this->YEAR->AdvancedSearch->getFromArray($filter)) {
            $this->YEAR->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->YEAR->AdvancedSearch->save();
        return true;
    }
