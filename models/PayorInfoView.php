 ...
 * Page class
 */
class PayorInfoView extends PayorInfo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PayorInfoView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("PAYOR_ID") ?? Route("PAYOR_ID")) !== null) {
                ...
                $this->RecKey["PAYOR_ID"] = $this->PAYOR_ID->QueryStringValue;
            } elseif (Post("PAYOR_ID") !== null) {
                $this->PAYOR_ID->setFormValue(Post("PAYOR_ID"));
                $this->RecKey["PAYOR_ID"] = $this->PAYOR_ID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->PAYOR_ID->setQueryStringValue($keyValue);
                $this->RecKey["PAYOR_ID"] = $this->PAYOR_ID->QueryStringValue;
            } else {
                $returnUrl = "PayorInfoList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "PayorInfoList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "PayorInfoList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
